...
theorem loopRepresentative_sq_eq_zero_of_square_top
  ...
  have hleSqX :
      QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal (x * x) ≤
        QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal x :=
    QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal_le_of_eq_mul rfl
  have hleXVertex :
      QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal x ≤
        QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal (G.vertex (target ell)) := by
    simpa [x, hLoop] using G.arrowIdeal_le_source ell
  have hleSqVertex :
      QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal (x * x) ≤
        QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal (G.vertex (target ell)) :=
  ...
  · obtain ⟨eSqProjective⟩ := hProjective
    let eSqVertex :
        QuotientSubmoduleEquidistribution.Tsukamoto.cyclicRightIdealFG (x * x) ≅
          QuotientSubmoduleEquidistribution.Tsukamoto.cyclicRightIdealFG
            (G.vertex (target ell)) :=
      eSqProjective ≪≫
    ...
    have hideals :
        QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal (x * x) =
          QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal
            (G.vertex (target ell)) :=
      QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal_eq_of_le_of_iso
        K hleSqVertex eSqVertex
    have hvertexMem :
        op (G.vertex (target ell)) ∈
          QuotientSubmoduleEquidistribution.Tsukamoto.principalRightIdeal (x * x) := by
      rw [hideals]
      exact Ideal.subset_span (Set.mem_singleton _)
    have hvertexJ2 : G.vertex (target ell) ∈ (Ring.jacobson B) ^ 2 :=
      QuotientSubmoduleEquidistribution.Tsukamoto.unop_mem_twoSidedIdeal_of_mem_principalRightIdeal
    ...
    exact G.vertex_ne_zero (target ell)
      (QuotientSubmoduleEquidistribution.Tsukamoto.idempotent_eq_zero_of_mem_jacobson
...
theorem algEquiv_b0_or_b1_of_incomingOneWayLollipop
    ...
    Nonempty (QuotientSubmoduleEquidistribution.LollipopConcrete.B0Model K ≃ₐ[K] B) ∨
      Nonempty (QuotientSubmoduleEquidistribution.LollipopConcrete.B1.B1Model K ≃ₐ[K] B) := by
  let E :=
  ...
  have hxsq : R.representative loopArrow * R.representative loopArrow = 0 :=
    QuotientSubmoduleEquidistribution.IndecomposableSkeleton.FaithfulCore.ConnectedSmallCore.loopRepresentative_sq_eq_zero
