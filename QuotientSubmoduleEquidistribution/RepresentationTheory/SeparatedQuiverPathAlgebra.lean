...
namespace QuotientSubmoduleEquidistribution.SeparatedQuiverPathAlgebra
...
def classifyPath :
    QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V) → BasisIndex V
  | ⟨a, _, .nil⟩ => Sum.inl a
...
def pathOfBasisIndex :
    BasisIndex V → QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V)
...
theorem pathOfBasisIndex_classifyPath
    (p : QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V)) :
...
def totalPathEquivBasisIndex :
    QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V) ≃ BasisIndex V where
...
def pathValue (p : QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V)) :
...
def toTriangularLinear :
    QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) →ₗ[K] TriangularAlgebra K V :=
  Finsupp.linearCombination K (pathValue K V)

omit [Fintype V] [∀ i j : V, Fintype (i ⟶ j)] in
@[simp]
theorem toTriangularLinear_single
    (p : QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V)) (k : K) :
    toTriangularLinear K V (QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single p k) =
...
theorem toTriangularLinear_ofPath
    (p : QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V)) :
    toTriangularLinear K V (QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.ofPath p) =
      pathValue K V p := by
  rw [QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.ofPath_eq_single, toTriangularLinear_single,
    one_smul]

/-- The trivial path at a plus vertex. -/
def plusVertexPath (i : V) : QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) :=
  QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.vertexIdempotent K (Sum.inl i)

/-- The trivial path at a minus vertex. -/
def minusVertexPath (i : V) : QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) :=
  QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.vertexIdempotent K (Sum.inr i)

/-- The separated length-one path attached to an original arrow. -/
def separatedArrowPath {i j : V} (e : i ⟶ j) :
    QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) :=
  QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.ofArrow
...
def fromTriangularLinear :
    TriangularAlgebra K V →ₗ[K] QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) where
  toFun r :=
...
theorem fromTriangularLinear_pathValue
    ...
    fromTriangularLinear K V (pathValue K V p) =
      QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.ofPath p := by
  obtain ⟨a, b, p⟩ := p
  cases p with
  | nil =>
      cases a with
...
theorem fromTriangularLinear_toTriangularLinear
    (f : QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V)) :
    fromTriangularLinear K V (toTriangularLinear K V f) = f := by
  induction f using QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.induction_linear with
  | zero => simp
  | add f g hf hg => rw [map_add, map_add, hf, hg]
  | single p k =>
      rw [toTriangularLinear_single, map_smul,
        fromTriangularLinear_pathValue,
        QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.ofPath_eq_single,
        QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.smul_single, mul_one]

/-- The separated path basis and the three triangular coordinate families
give a linear equivalence. -/
def linearEquiv :
    QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) ≃ₗ[K] TriangularAlgebra K V where
...
theorem pathValue_mul_pathValue
    (x y : QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V)) :
    ...
    · simp only [pathValue_pathOfBasisIndex, basisValue]
      ...
      · subst b
        ...
        change vertexValue K V a =
          (QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath.mul?
            (⟨a, a, Quiver.Path.nil⟩ :
              QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V))
            ⟨a, a, Quiver.Path.nil⟩).elim 0 (pathValue K V)
        rw [QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath.mul?_mk,
          _root_.Quiver.Path.comp_nil]
        rfl
      · rw [if_neg h,
          QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath.mul?_eq_none (fun hba => h hba.symm)]
        rfl
    · simp only [pathValue_pathOfBasisIndex, basisValue]
      ...
      · subst a
        ...
        rfl
      · rw [if_neg h,
          QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath.mul?_eq_none (fun hla => h hla.symm)]
        rfl
  · rcases cy with b | ⟨k, l, f⟩
    · simp only [pathValue_pathOfBasisIndex, basisValue]
      ...
      · subst b
        ...
        rfl
      · rw [if_neg h, QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath.mul?_eq_none h]
        rfl
    · simp only [pathValue_pathOfBasisIndex, basisValue]
      rw [arrowValue_mul_arrowValue,
        QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath.mul?_eq_none (by
...
theorem toTriangularLinear_single_mul_single
    (x y : QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath (Vertex V)) (a b : K) :
    toTriangularLinear K V
        (QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single x a *
          QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single y b) =
      toTriangularLinear K V (QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single x a) *
        toTriangularLinear K V (QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single y b) := by
  rw [QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single_mul_single]
...
theorem toTriangularLinear_mul
    (f g : QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V)) :
  ...
  | single x a =>
      induction g using QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.induction_linear with
      | zero => simp
      | add g₁ g₂ hg₁ hg₂ =>
...
theorem toTriangularLinear_one :
    toTriangularLinear K V (1 : QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V)) = 1 := by
  let x : QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) :=
    (linearEquiv K V).symm (1 : TriangularAlgebra K V)
  calc
    toTriangularLinear K V (1 : QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V)) =
...
def algebraEquiv :
    QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V) ≃ₐ[K] TriangularAlgebra K V :=
...
def moduleEquivalence :
    CategoryTheory.Equivalence
      ...
      (ModuleCat.{uV} (QuotientSubmoduleEquidistribution.Foundation.pathAlgebra K (Vertex V))) :=
...
end QuotientSubmoduleEquidistribution.SeparatedQuiverPathAlgebra
