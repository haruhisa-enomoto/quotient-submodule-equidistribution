...
theorem diagonal_projection_compatible
    ...
    ξ.comp
        (Ext.mk₀
          (QuotientSubmoduleEquidistribution.YonedaExtReflection.blockDiagonalBiprodEnd
            (𝟙 P) (0 : Q ⟶ Q)))
        (add_zero 1) =
      (Ext.mk₀
          (QuotientSubmoduleEquidistribution.YonedaExtReflection.blockDiagonalBiprodEnd
...
theorem offdiagonal_entries_not_both_zero
  ...
  have hp : p ≫ p = p := by
    dsimp [p]
    rw [QuotientSubmoduleEquidistribution.YonedaExtReflection.blockDiagonalBiprodEnd_comp]
    simp
  have hq : q ≫ q = q := by
    dsimp [q]
    rw [QuotientSubmoduleEquidistribution.YonedaExtReflection.blockDiagonalBiprodEnd_comp]
  ...
  rcases
      QuotientSubmoduleEquidistribution.YonedaExtReflection.endpoint_idempotents_trivial_of_extClass_compatibility
        hS hM p q hp hq hcompat with hzero | hone
  · exact hP <|
      (QuotientSubmoduleEquidistribution.YonedaExtReflection.blockDiagonalBiprodEnd_eq_zero_iff
        (𝟙 P) (0 : Q ⟶ Q)).mp hzero.1 |>.1
  · exact hQ <|
      ((QuotientSubmoduleEquidistribution.YonedaExtReflection.blockDiagonalBiprodEnd_eq_id_iff
...
theorem false_of_binary_ext_degree_one
  ...
  have hRowSClass :
    ...
    QuotientSubmoduleEquidistribution.ExtForkSemisimpleLayers.biprod_inl_comp_extClass_ne_zero
  ...
  have hColPClass :
    ...
    QuotientSubmoduleEquidistribution.ExtForkSemisimpleLayers.extClass_comp_biprod_fst_ne_zero
...
variable {K A : Type u}
  ...
  [Algebra K A]
  {κ : Type i} [Finite κ]
  (τ : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, i, u} A κ)
...
theorem false_of_two_distinct_simple_kernel_and_quotient
    ...
    (hM : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule A M) :
    False := by
...
variable {R : Type u} [Ring R] [IsNoetherianRing R]
  {κ : Type i}
  (τ : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, i, u} R κ)
...
theorem exists_nonzero_uniserial_submodule_of_projective_boundary
    (hProjective :
      QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsProjectiveNakayamaSkeleton τ)
    (i : κ) :
    ∃ U : Submodule R (τ.obj i),
      U ≠ ⊥ ∧ QuotientSubmoduleEquidistribution.IsUniserialModule R U := by
  classical
  obtain ⟨P⟩ :=
    QuotientSubmoduleEquidistribution.NakayamaRepresentationFiniteBridge.inFac_projectiveLabels
      τ (τ.obj i)
  ...
  have hSource : QuotientSubmoduleEquidistribution.IsUniserialModule R (τ.obj (P.label t)) :=
...
theorem exists_maximal_nonzero_uniserial_submodule_of_projective_boundary
    ...
    ∃ U : Submodule R (τ.obj i),
      U ≠ ⊥ ∧
        QuotientSubmoduleEquidistribution.IsUniserialModule R U ∧
          ∀ V : Submodule R (τ.obj i),
            U ≤ V →
              V ≠ ⊥ →
                QuotientSubmoduleEquidistribution.IsUniserialModule R V → V ≤ U := by
  obtain ⟨U, hUne, hU⟩ :=
  ...
  obtain ⟨V, hUV, hVmax⟩ :=
    exists_maximal_ge_of_wellFoundedGT
      (fun W : Submodule R (τ.obj i) ↦
        W ≠ ⊥ ∧ QuotientSubmoduleEquidistribution.IsUniserialModule R W)
      U ⟨hUne, hU⟩
    ...
    U ≠ ⊥ →
      QuotientSubmoduleEquidistribution.IsUniserialModule R U →
        (∀ V : Submodule R (τ.obj i),
          U ≤ V →
            V ≠ ⊥ →
              QuotientSubmoduleEquidistribution.IsUniserialModule R V → V ≤ U) →
...
theorem isNakayamaSkeleton_of_projective_boundary_of_maximal_uniserial_splits
    ...
    QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsNakayamaSkeleton τ := by
  intro i
...
theorem exists_nonzero_uniserial_quotient_of_injective_boundary
    (hInjective :
      QuotientSubmoduleEquidistribution.SerialRingBridge.IsInjectiveNakayamaSkeleton τ)
    (i : κ) :
    ∃ U : FGModuleCat.{u} R,
      Nontrivial U ∧
        QuotientSubmoduleEquidistribution.IsUniserialModule R U ∧
  ...
  classical
  have hCogenerates :
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.InSubOfModule
  ...
  letI : Epi q :=
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective q).mpr
      hqSurjective
  have hTarget :
      QuotientSubmoduleEquidistribution.IsUniserialModule R (τ.obj (P.label t)) :=
    hInjective (P.label t) (P.mem t)
  have hU : QuotientSubmoduleEquidistribution.IsUniserialModule R U :=
    hTarget.submodule V
  haveI : Nontrivial U :=
...
theorem hom_to_uniserial_factor_comparable
    [CategoryTheory.Projective P]
    (hX : QuotientSubmoduleEquidistribution.IsUniserialModule R X)
    (f g : P ⟶ X) :
    (∃ a : P ⟶ P, a ≫ g = f) ∨
      (∃ b : P ⟶ P, b ≫ f = g) := by
  unfold QuotientSubmoduleEquidistribution.IsUniserialModule at hX
  rcases hX.total
