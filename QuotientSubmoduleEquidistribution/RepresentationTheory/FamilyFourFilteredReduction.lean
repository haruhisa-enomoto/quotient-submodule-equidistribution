...
namespace QuotientSubmoduleEquidistribution.FamilyFourFilteredReduction
...
theorem quotient_inAdd_pair_of_nonzero_submodule
  ...
  · exfalso
    ...
    have hfinj : Function.Injective f.hom.hom :=
      (_root_.QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_mono_iff_injective f).mp
        inferInstance
    apply hN
    ext z
    constructor
...
theorem secondRadical_eq_bot_of_inAdd_lengthTwo_simple
  ...
  let e :
    ...
    QuotientSubmoduleEquidistribution.radicalRestrictionEquiv
...
theorem secondRadical_le_every_nonzero_submodule
  ...
  let q₂ :
      Module.jacobson R (Module.jacobson R (sigma.obj x)) →ₗ[R]
        Module.jacobson R (Module.jacobson R Q) :=
    QuotientSubmoduleEquidistribution.radicalRestriction
      (QuotientSubmoduleEquidistribution.radicalRestriction q)
  intro z hz
  obtain ⟨j, hj, hjz⟩ := hz
...
end QuotientSubmoduleEquidistribution.FamilyFourFilteredReduction
