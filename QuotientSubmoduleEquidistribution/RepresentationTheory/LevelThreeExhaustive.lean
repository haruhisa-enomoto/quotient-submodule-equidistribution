...
`paper/quotient_submodule_equidistribution/level-four.tex`.  The first part proves the unconditional
...
theorem moduleRadical_isSimple_of_oneNonsimple
  ...
  obtain ⟨j, f, hf, hrad⟩ :=
    QuotientSubmoduleEquidistribution.LevelTwoUnconditional.exists_indecomposable_quotient_with_simple_radical
      σ hxn
  letI : Epi f := hf
  have hjmem : j ∈ S :=
