...
theorem moduleTop_isSimple_of_twoTargetModel
    ...
    (f : V →ₗ[K] W) (g : V →ₗ[K] Z)
    (hfg : QuotientSubmoduleEquidistribution.LoewyTwoRankCore.IsTwoTargetIdempotentIndecomposable
      f g)
    (hsourceLength :
  ...
  have hsource :
    ...
    QuotientSubmoduleEquidistribution.LoewyTwoRankCore.twoTarget_source_finrank_le_one hfg
  have hlengthLe :
...
theorem moduleTop_isSimple_of_inFac_gabrielCommonSourcePair
  ...
  have htopIsotypic :
    ...
    σ.moduleTop_isIsotypicOfType_of_inFac_commonSourcePair
      (fun e ↦
        QuotientSubmoduleEquidistribution.GabrielArrowBridge.LengthTwo.quotient
          (A := Aᵐᵒᵖ) σ e)
      p hj
