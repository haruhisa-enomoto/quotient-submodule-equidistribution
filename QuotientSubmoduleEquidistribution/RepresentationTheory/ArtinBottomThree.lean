...
namespace QuotientSubmoduleEquidistribution.ArtinBottomThree
...
local instance (I : TwoSidedIdeal R) :
  ...
  QuotientSubmoduleEquidistribution.ArtinBottomTwo.factorOppositeNoetherian_of_oppositeNoetherian
    R I

/-- The canonical factor skeleton used throughout the direct formula. -/
abbrev factorSkeleton (I : TwoSidedIdeal R) :=
  QuotientSubmoduleEquidistribution.ArtinBottomTwo.factorSkeleton (R := R) I
...
variable {ι : Type (u + 1)}
  ...
  (duality : ∀ I : TwoSidedIdeal R,
    QuotientSubmoduleEquidistribution.ArtinDuality.Data (Quotient.Factor I))

/-- Ringel's core equivalence on every canonical factor skeleton. -/
def factorRingelCoreEquiv (I : TwoSidedIdeal R) :=
  QuotientSubmoduleEquidistribution.ArtinDuality.ringelCoreEquiv
...
def quotientTripleCoreSplitEquiv :
  ...
  qTripleCoreSplitEquiv σ
    (fun I : TwoSidedIdeal R ↦ factorSkeleton (R := R) I)
    (fun I ↦
      QuotientSubmoduleEquidistribution.QuotientSkeletonAlignment.artinInflationData σ I)
    (fun I ↦
      QuotientSubmoduleEquidistribution.ArtinDuality.faithfulNormalForm
        (duality I) (factorSkeleton (R := R) I))
...
end QuotientSubmoduleEquidistribution.ArtinBottomThree
