...
namespace QuotientSubmoduleEquidistribution.NakayamaFixedTopChains
...
theorem uniserialTopIndex_projectiveLabelOfSimple
    ...
    (j : σ.SimpleIndex) :
    QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.uniserialTopIndex σ
  ...
  apply σ.eq_of_iso
  let eSelected :=
    QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.uniserialTopIso σ
      (projectiveLabelOfSimple σ j)
...
theorem exists_fixedTop_quotient_label
    ...
    ∃ k : ι,
      (∃ f :
        ...
        Epi f) ∧
      QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.uniserialTopIndex σ k
          (hNakayama k) = j ∧
  ...
  have hQindec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R Q :=
  ...
  have htop :
    ...
    exact
      (QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.uniserialTopIndex_eq_of_epi
...
def fixedTopLabel
    (hNakayama : ∀ i : ι, IsUniserialModule R (σ.obj i))
    (p : QuotientSubmoduleEquidistribution.NakayamaCombinatorics.ChainPoint
...
theorem fixedTopLabel_surjective
  ...
  intro i
  let j :=
    QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.uniserialTopIndex σ i
      (hNakayama i)
  obtain ⟨f, hf⟩ :=
    QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.exists_epi_projectiveLabelOfSimple_of_top_iso
      σ i (hNakayama i) j
      (QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.uniserialTopIso σ i
        (hNakayama i))
  letI : Epi f := hf
  have hle : σ.compositionLength i ≤ fixedTopCapacity σ j := by
    exact σ.compositionLength_le_of_epi f
  let p : QuotientSubmoduleEquidistribution.NakayamaCombinatorics.ChainPoint
      (fixedTopCapacity σ) :=
  ...
  have hlength :
    ...
    omega
  exact
    QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.eq_of_top_iso_of_compositionLength_eq_of_projective_uniserial
...
open QuotientSubmoduleEquidistribution.NakayamaCombinatorics
...
theorem exists_epi_iff_fixedTop_chainClosure
  ...
  rw [QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.exists_epi_iff_topIndex_eq_and_compositionLength_le]
...
def fixedTopChainData
    ...
    (hNakayama : ∀ i : ι, IsUniserialModule R (σ.obj i)) :
    QuotientSubmoduleEquidistribution.NakayamaModuleChains.FixedTopChainData σ
...
end QuotientSubmoduleEquidistribution.NakayamaFixedTopChains
