  ...
  · have hni : ¬ Injective
    ...
    have hσ : Oσ.reverse.successor a = (Oσ.tau ⟨a, hp⟩).1 := by
      simp [QuotientSubmoduleEquidistribution.BoundaryTranslationChains.Data.successor,
        QuotientSubmoduleEquidistribution.BoundaryTranslationChains.Data.reverse, hp]
    have hτ : Oτ.successor (D.pushforwardIrreduciblePair σ τ a) =
        (Oτ.tau.symm
          ⟨D.pushforwardIrreduciblePair σ τ a, hni⟩).1 := by
      simp [QuotientSubmoduleEquidistribution.BoundaryTranslationChains.Data.successor, hni]
    rw [hσ, hτ]
