...
namespace QuotientSubmoduleEquidistribution.NoLoopNakayamaReduction
...
theorem isUniserialModule_of_simpleTop_of_radicalUniserial
    ...
    (hRadical :
      QuotientSubmoduleEquidistribution.IsUniserialModule R (Module.jacobson R M)) :
    QuotientSubmoduleEquidistribution.IsUniserialModule R M := by
  let J : Submodule R M := Module.jacobson R M
  unfold QuotientSubmoduleEquidistribution.IsUniserialModule at hRadical ⊢
  constructor
  intro P Q
  by_cases hPTop : P = ⊤
  · right
...
end QuotientSubmoduleEquidistribution.NoLoopNakayamaReduction
