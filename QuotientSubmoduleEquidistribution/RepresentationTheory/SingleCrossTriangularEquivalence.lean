...
namespace QuotientSubmoduleEquidistribution.A2Triangular
...
end QuotientSubmoduleEquidistribution.A2Triangular
...
def diagonalAlgHom {i j : Fin 2} (hij : i ≠ j) :
    QuotientSubmoduleEquidistribution.A2Triangular.Base K →ₐ[K] B where
...
def singleCrossTriangularAlgHom
    ...
    QuotientSubmoduleEquidistribution.A2Triangular.Model K →ₐ[K] B := by
  ...
  refine TrivSqZeroExt.lift (D.diagonalAlgHom hij)
    ((LinearMap.toSpanSingleton K B a).comp
      (QuotientSubmoduleEquidistribution.A2Triangular.Arrow.linearEquiv K).toLinearMap) ?_ ?_ ?_
  · intro x y
...
theorem singleCrossTriangularAlgHom_apply
    ...
    (hR : a * D.liftedCoordinate j = a)
    (hdata : SingleCrossRadicalData (K := K) a)
    (x : QuotientSubmoduleEquidistribution.A2Triangular.Model K) :
...
theorem singleCrossTriangularAlgHom_bijective
  ...
  · intro b
    ...
    obtain ⟨c, hc⟩ := hzSpan
    refine ⟨(((cj, ci), ⟨c⟩) : QuotientSubmoduleEquidistribution.A2Triangular.Model K), ?_⟩
    rw [D.singleCrossTriangularAlgHom_apply]
...
noncomputable def singleCrossTriangularAlgEquiv
    ...
    (hR : a * D.liftedCoordinate j = a)
    (hdata : SingleCrossRadicalData (K := K) a) :
    QuotientSubmoduleEquidistribution.A2Triangular.Model K ≃ₐ[K] B :=
