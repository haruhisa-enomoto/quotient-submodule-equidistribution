...
theorem arrows_of_fixedStripRise
    ...
    (h : QuotientSubmoduleEquidistribution.FixedStripCyclicBalance.IsRise
...
theorem fixedStripDeleted_isProjectivelyRooted_of_isRise
  ...
  have hpa : QuotientSubmoduleEquidistribution.RootedDigraph.ReachInside
      σ.irreducibleEdge Deleted C.p X.x :=
    Relation.ReflTransGen.tail Relation.ReflTransGen.refl
      ⟨hp, ha, hwalls.2⟩
  have hpac : QuotientSubmoduleEquidistribution.RootedDigraph.ReachInside
      σ.irreducibleEdge Deleted C.p C.center.c :=
    hpa.tail ⟨ha, hc, X.x_to_c⟩
  have hpacz : QuotientSubmoduleEquidistribution.RootedDigraph.ReachInside
