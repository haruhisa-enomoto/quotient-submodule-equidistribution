...
namespace QuotientSubmoduleEquidistribution.AnnihilatorInflation
...
private theorem smul_sumOver_eq_zero
  ...
  let E := FGModuleCat.isoToLinearEquiv
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.biproductIsoPiFG
...
theorem supportAnnihilator_sSet (S : Set K) :
  ...
  · intro hr j hj x
    ...
    apply
      (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_mono_iff_injective P.map).mp
        inferInstance
    rw [P.map.hom.hom.map_smul]
...
theorem supportAnnihilator_asIdeal_eq_annihilator_basicModule
    (B : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.FiniteSupport (ι := K)) :
...
abbrev RealizedAnnihilator :=
  QuotientSubmoduleEquidistribution.BottomLevels.FiniteJointStratification.JointRange
...
noncomputable def qStratum (C : QClosed sigma) :
  ...
  exact
    if h : qClosedAnnihilator sigma C = ⊥ then none
      ...
      some
        ⟨QuotientSubmoduleEquidistribution.BottomLevels.FiniteJointStratification.toJointLeft
...
noncomputable def sStratum (C : SClosed sigma) :
  ...
  exact
    if h : sClosedAnnihilator sigma C = ⊥ then none
      ...
      some
        ⟨QuotientSubmoduleEquidistribution.BottomLevels.FiniteJointStratification.toJointRight
            (qClosedAnnihilator sigma) (sClosedAnnihilator sigma) C,
          h⟩
...
private theorem qFaithfulLevelCard (n : ℕ) :
  ...
  unfold QuotientSubmoduleEquidistribution.BottomLevels.MinimalFaithfulCore.faithfulLevelCount
...
end QuotientSubmoduleEquidistribution.AnnihilatorInflation
