...
namespace QuotientSubmoduleEquidistribution.FiniteLengthDecomposition
...
structure PiDecomposition (X : FGModuleCat.{w} R) where
  ...
  indecomposable :
    ∀ i, QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R (summand i)
...
def singleton (X : FGModuleCat.{w} R)
    (hX : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R X) :
    PiDecomposition X where
...
theorem exists_biproduct_decomposition
    ...
    ∃ (J : FintypeCat.{0}) (Y : J → FGModuleCat.{w} R),
      (∀ j, QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R (Y j)) ∧
...
end QuotientSubmoduleEquidistribution.FiniteLengthDecomposition
