...
abbrev LeftFiniteProjectives :=
  QuotientSubmoduleEquidistribution.CPSLeftStandardLayers.LeftFiniteProjectives R
...
def fgProjectiveHomDuality [IsNoetherianRing Rᵐᵒᵖ] :
  ...
  (fgProjectivesEquivFiniteProjectives (R := R)).op |>.trans
    ((QuotientSubmoduleEquidistribution.CPSLeftStandardLayers.regularHomDualityEquivalence R).rightOp.symm) |>.trans
...
private theorem reverseDual_injective_inverseImage :
  ...
  | op X =>
      exact
        ((QuotientSubmoduleEquidistribution.Contragredient.reverseDualityEquivalence K R).map_injective_iff
          (Opposite.op X)).trans
...
theorem indecomposable_map_fgFullSubcategory
    ...
    QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R (E.functor.obj M).obj := by
