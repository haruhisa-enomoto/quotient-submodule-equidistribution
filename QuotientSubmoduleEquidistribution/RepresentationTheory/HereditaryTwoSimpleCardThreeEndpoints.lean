...
theorem lengthTwoProjective_endpoints
  ...
  haveI : Epi (FGModuleCat.ofHom (sigma.moduleRadical pLong.1).mkQ) :=
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective _).mpr
  ...
  have hsource : LengthTwo.source sigma xLong = long := by
    apply Subtype.ext
    exact
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.SimpleQuotient.index_eq_of_compositionLength_eq_two
  ...
  haveI :
    ...
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_mono_iff_injective _).mpr
  ...
  have htarget : LengthTwo.target sigma xLong = short := by
    apply Subtype.ext
    exact
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.SimpleSubmodule.index_eq_of_compositionLength_eq_two
