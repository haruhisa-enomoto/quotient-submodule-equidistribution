...
namespace QuotientSubmoduleEquidistribution.A2Separated
...
theorem top_isIndecomposable_of_bisupported
    ...
    [Nontrivial D.top] :
    QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule K D.top := by
...
private theorem field_isIndecomposable :
    QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule K K := by
...
theorem candidateFG_indecomposable (i : Fin 3) :
    QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule (Triangular K) (candidateFG K i) := by
  fin_cases i
  ·
    exact (realized_isIndecomposable_iff (arrowData K)).2
      (arrowData_isIndecomposableSeparatedData K)
  ·
    change QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule (Triangular K)
      (Realized (topData K))
...
theorem candidateFG_simple_iff (i : Fin 3) :
  ...
  · constructor
    · intro hs
      ...
      exact (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.simple_iff_isSimpleModule_fg
  ...
  · constructor
    · intro _
      exact Or.inl rfl
    · intro _
      apply (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.simple_iff_isSimpleModule_fg
        (candidateFG K 1)).2
...
theorem exists_candidateFG_iso
    ...
    (hX : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule (Triangular K) X) :
  ...
  · letI : Subsingleton D.top := htop
    have hind : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule K D.radical :=
  ...
  · letI : Nontrivial D.top := htop
    ...
    · letI : Subsingleton D.radical := hrad
      have hind : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule K D.top :=
...
noncomputable abbrev sigma :
  ...
  indecomposableSkeletonOfFiniteLength
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fgModule_isFiniteLength_of_finiteDimensional
      K (Triangular K))
...
def modelToTriangularAlgHom :
    QuotientSubmoduleEquidistribution.A2Triangular.Model K →ₐ[K] Triangular K where
...
theorem modelToTriangularAlgHom_bijective :
  ...
  · intro x y hxy
    ...
    · apply QuotientSubmoduleEquidistribution.A2Triangular.Arrow.ext
      simpa [modelToTriangularAlgHom] using
        congrArg (fun z : Triangular K ↦ z.snd.val) hxy
  · intro z
    refine ⟨((z.fst.2, z.fst.1),
      QuotientSubmoduleEquidistribution.A2Triangular.Arrow.mk z.snd.val), ?_⟩
    apply TrivSqZeroExt.ext
    · apply Prod.ext <;> rfl
    · apply SeparatedIdeal.ext
      rfl
...
noncomputable def modelAlgEquiv :
    QuotientSubmoduleEquidistribution.A2Triangular.Model K ≃ₐ[K] Triangular K :=
...
noncomputable def node :
    QuotientSubmoduleEquidistribution.BottomLevels.FiniteDimensionalRecurrence.AlgebraNode K where
...
end QuotientSubmoduleEquidistribution.A2Separated
