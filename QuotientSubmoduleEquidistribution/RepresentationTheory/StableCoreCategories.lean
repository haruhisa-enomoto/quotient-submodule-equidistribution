...
theorem torsionless_of_inSubOfModule
    ...
    (hX : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.InSubOfModule G X) :
    Torsionless X := by
  obtain ⟨L, m, hm⟩ := hX
...
theorem cotorsionless_of_inFacOfModule
    ...
    (hX : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.InFacOfModule J X) :
    Cotorsionless X := by
  obtain ⟨L, p, hp⟩ := hX
