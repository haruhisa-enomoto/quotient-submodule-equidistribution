...
namespace QuotientSubmoduleEquidistribution.ExtDegreeNakayamaReduction
...
theorem isUniserialModule_congr
    ...
    (e : M ≃ₗ[R] N)
    (hM : QuotientSubmoduleEquidistribution.IsUniserialModule R M) :
    QuotientSubmoduleEquidistribution.IsUniserialModule R N := by
  unfold QuotientSubmoduleEquidistribution.IsUniserialModule at hM ⊢
  constructor
  intro P Q
  rcases hM.total
...
theorem isUniserialModule_of_subsingleton
    ...
    [Subsingleton M] :
    QuotientSubmoduleEquidistribution.IsUniserialModule R M := by
  unfold QuotientSubmoduleEquidistribution.IsUniserialModule
  constructor
  intro P Q
  left
...
theorem isNakayamaSkeleton_of_simpleTops_of_indecRadicals
  ...
  · have hJIndec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R J :=
    ...
    have hRepUniserial :
        QuotientSubmoduleEquidistribution.IsUniserialModule R (σ.obj j) :=
      ih j hJlt
...
theorem indecRadicals_of_isNakayamaSkeleton
  ...
  letI : Nontrivial J := by
    ...
    exact hJ
  have hJUniserial : QuotientSubmoduleEquidistribution.IsUniserialModule R J :=
    QuotientSubmoduleEquidistribution.IsUniserialModule.submodule (hNakayama i) J
...
theorem epi_of_comp_radicalQuotient_epi_of_simpleTop
    ...
    Epi f := by
  apply
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective f).mpr
...
theorem isNakayamaSkeleton_of_simpleTops_of_projectiveNakayama
  ...
  have hTopIndec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R Top :=
  ...
  have hQuotUniserial :
    ...
    QuotientSubmoduleEquidistribution.IsUniserialModule.quotient
...
variable {K A : Type u}
  ...
  [Algebra K A]
  {κ : Type v} [Finite κ]
  (τ : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, v, u} A κ)
...
structure ExtGabrielForkExtraction : Prop where
  ...
  outgoing_of_decomposable_radical :
    ∀ i : κ,
      ...
      ¬ QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule A
          (Module.jacobson A (τ.obj i)) →
        HasOutgoingExtGabrielFork (K := K) τ
...
theorem extGabrielInOutDegreeOneNakayamaClassification_of_forkExtraction
    (hFork : ExtGabrielForkExtraction (K := K) τ) :
    QuotientSubmoduleEquidistribution.NoLoopNakayamaReduction.ExtGabrielInOutDegreeOneNakayamaClassification
      (K := K) τ := by
...
end QuotientSubmoduleEquidistribution.ExtDegreeNakayamaReduction
