import QuotientSubmoduleEquidistribution.RepresentationTheory.AlmostSplitDuality
...
theorem exists_leftAlmostSplit_of_finiteDimensional
  ...
  let y := QuotientSubmoduleEquidistribution.Contragredient.dualMapLabel
    K R sigma tau x
  have hy : ¬ Projective (tau.obj y) := by
    ...
    have hdual : Projective
      ...
      Projective.of_iso
        (QuotientSubmoduleEquidistribution.Contragredient.dualMapObjIso
          K R sigma tau x).symm hprojective
    have hop : Projective (Opposite.op (sigma.obj x)) :=
      ((QuotientSubmoduleEquidistribution.Contragredient.dualityEquivalence K R).map_projective_iff
  ...
  let D := QuotientSubmoduleEquidistribution.Contragredient.reverseDualityEquivalence K R
  have hdual : IsLeftAlmostSplit (D.functor.map q.op) :=
    hq.map_op_equivalence D
  have hyx :
      QuotientSubmoduleEquidistribution.Contragredient.reverseMapLabel
          K R sigma tau y = x := by
    exact QuotientSubmoduleEquidistribution.Contragredient.reverseMapLabel_dualMapLabel
      K R sigma tau x
  let sourceIso : sigma.obj x ≅
      D.functor.obj (Opposite.op (tau.obj y)) :=
    eqToIso (congrArg sigma.obj hyx.symm) ≪≫
      (QuotientSubmoduleEquidistribution.Contragredient.reverseMapObjIso
...
theorem finiteDimensional_cofiniteTwoDeletionSets_finite :
  ...
  exact sigma.moduleFiniteArtin_cofiniteTwoDeletionSets_finite
    (k := K) (QuotientSubmoduleEquidistribution.ArtinDuality.ofFiniteDimensional K)
    (fun z ↦
      sigma.minimalRightAlmostSplitDecomposition_nonempty_of_finiteDimensional_of_not_projective
        K R z.1 z.2)
    (fun z ↦
