...
def factorThroughAddIdeal (S T : Set ι) :
    QuotientSubmoduleEquidistribution.CategoricalIdeal.HomIdeal (σ.AddCategory S) where
  hom X Y :=
...
theorem quotientCategory_hasZeroRadical
  ...
  let F := CategoryTheory.Quotient.functor I.rel
  apply
    QuotientSubmoduleEquidistribution.CategoricalRadical.hasZeroRadical_of_isAdditivelyGeneratedBy
