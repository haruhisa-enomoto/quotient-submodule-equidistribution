...
namespace QuotientSubmoduleEquidistribution.GabrielArrowBridge
...
theorem extClass_ne_zero_of_indec_middle
    ...
    (hM : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule A S.X₂) :
    hS.extClass ≠ 0 := by
  intro hzero
...
private def topLinearEquiv (x : σ.LengthTwoIndex) :
  ...
  letI : IsSimpleModule A (σ.obj Q.index) :=
    (_root_.QuotientSubmoduleEquidistribution.IndecomposableSkeleton.simple_iff_isSimpleModule_fg _).mp
      Q.simple
  letI : Epi Q.map := Q.epi
  have hsurj : Function.Surjective Q.map.hom.hom :=
    (_root_.QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective Q.map).mp
      inferInstance
...
private def radicalLinearEquiv (x : σ.LengthTwoIndex) :
  ...
  (Classical.choice
    (QuotientSubmoduleEquidistribution.LengthTwoGabrielBridge.IndecomposableSkeleton.moduleRadical_linearEquiv_simpleSubmodule
...
theorem toGabrielArrow_injective
  ...
  have hcompat :
    ...
    exact hcclass
  obtain ⟨a₂, ha₁₂, ha₂₃⟩ :=
    QuotientSubmoduleEquidistribution.YonedaExtReflection.exists_middle_morphism_of_extClass_compatibility
      h₁ h₂ a₁ a₃ hcompat
  let φ : S₁ ⟶ S₂ :=
    { τ₁ := a₁
      τ₂ := a₂
...
theorem toGabrielArrow_surjective :
  ...
  let M : FGModuleCat.{u} A :=
    FGModuleCat.of A
      (QuotientSubmoduleEquidistribution.NoParallelExtOne.PushoutExtension.middle p f)
  have hMindec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule A M := by
    apply QuotientSubmoduleEquidistribution.NoParallelExtOne.indecomposable_middle_of_extClass_ne_zero
      (QuotientSubmoduleEquidistribution.NoParallelExtOne.PushoutExtension.shortExact p hp f)
    rw [hfclass]
    exact hξne
  obtain ⟨j, ⟨e⟩⟩ := σ.complete M hMindec
  let S := QuotientSubmoduleEquidistribution.NoParallelExtOne.PushoutExtension.shortComplex p f
  ...
  let qLinear : σ.obj j →ₗ[A] σ.obj s.1 :=
    (QuotientSubmoduleEquidistribution.NoParallelExtOne.PushoutExtension.projection p f).comp
      (FGModuleCat.isoToLinearEquiv e).symm.toLinearMap
  have hqSurj : Function.Surjective qLinear := by
    intro z
    obtain ⟨m, hm⟩ :=
      QuotientSubmoduleEquidistribution.NoParallelExtOne.PushoutExtension.projection_surjective
        p hp f z
  ...
  let Q : σ.SimpleQuotient j :=
    { index := s.1
      ...
      map := qMap
      epi :=
        (_root_.QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective qMap).mpr
          hqSurj }
  let mLinear : σ.obj t.1 →ₗ[A] σ.obj j :=
    (FGModuleCat.isoToLinearEquiv e).toLinearMap.comp
      (QuotientSubmoduleEquidistribution.NoParallelExtOne.PushoutExtension.inclusion p f)
  have hmInj : Function.Injective mLinear :=
    (FGModuleCat.isoToLinearEquiv e).injective.comp
      (QuotientSubmoduleEquidistribution.NoParallelExtOne.PushoutExtension.inclusion_injective p f)
  ...
  let T : σ.SimpleSubmodule j :=
    { index := t.1
      ...
      map := mMap
      mono :=
        (_root_.QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_mono_iff_injective mMap).mpr
          hmInj }
  let x : σ.LengthTwoIndex := ⟨j, hj⟩
  have hsource : source σ x = s := by
    apply Subtype.ext
    exact
      _root_.QuotientSubmoduleEquidistribution.IndecomposableSkeleton.SimpleQuotient.index_eq_of_compositionLength_eq_two
        σ hj (quotient σ x) Q
  have htarget : target σ x = t := by
    apply Subtype.ext
    exact
      _root_.QuotientSubmoduleEquidistribution.IndecomposableSkeleton.SimpleSubmodule.index_eq_of_compositionLength_eq_two
...
theorem noParallelExtSupport_of_finiteDimensional_of_finiteSkeleton :
    ...
    ∀ {ι : Type v} [Finite ι]
      (σ : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, v, u} Bᵐᵒᵖ ι),
...
theorem qLevelCount_two_formula_gabrielArrowCount :
  ...
  letI : IsNoetherianRing Bᵐᵒᵖ :=
    QuotientSubmoduleEquidistribution.isNoetherianRing_op_of_finiteDimensional K B
  intro ι _ σ
  rw [QuotientSubmoduleEquidistribution.LevelTwoUnconditional.qLevelCount_two_formula_of_finiteDimensional_of_finiteSkeleton
      K B σ,
...
theorem sLevelCount_two_formula_gabrielArrowCount :
  ...
  letI : IsNoetherianRing Bᵐᵒᵖ :=
    QuotientSubmoduleEquidistribution.isNoetherianRing_op_of_finiteDimensional K B
  intro ι _ σ
  rw [QuotientSubmoduleEquidistribution.LevelTwoUnconditional.sLevelCount_two_formula_of_finiteDimensional_of_finiteSkeleton
      K B σ,
    natCard_lengthTwoIndex_eq_gabrielArrowCount K B σ]
...
theorem qAndSLevelCount_two_formula_gabrielArrowCount_of_rightRepresentationFinite
    ...
    (hrep : QuotientSubmoduleEquidistribution.IsRightRepresentationFinite.{u, u, u} K A) :
    ...
    let τ :
        QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, u, u}
          Aᵐᵒᵖ ι :=
      QuotientSubmoduleEquidistribution.LevelTwoUnconditional.relabelIndecomposableSkeleton σ e
    (σ.qClosure.levelCount 2 =
  ...
  let E :
    ...
    QuotientSubmoduleEquidistribution.LevelTwoUnconditional.relabelAlignedEquivalence σ e
  have hqτ :
...
end QuotientSubmoduleEquidistribution.GabrielArrowBridge
