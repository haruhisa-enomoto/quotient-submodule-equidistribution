...
theorem exists_leftComponent_not_mem_pow {x y : ι}
    (Q : QuotientSubmoduleEquidistribution.CategoricalRadical.NilpotentRadicalData
      (FGModuleCat.{u} R))
    (A : σ.MinimalRightAlmostSplitDecomposition y)
    (f : σ.obj x ⟶ σ.obj y)
...
theorem transGen_irreducibleEdge_of_mem_radical_not_mem_pow_succ
  ...
  | n + 1, x, y, f, hfrad, hf => by
      ...
      by_cases ha :
          QuotientSubmoduleEquidistribution.CategoricalRadical.IsRadicalMorphism a
...
theorem transGen_irreducibleEdge_of_nonzeroNonisomorphism
    ...
    (Q : QuotientSubmoduleEquidistribution.CategoricalRadical.NilpotentRadicalData
      (FGModuleCat.{u} R))
    {x y : ι}
    (h : QuotientSubmoduleEquidistribution.RepresentationDirected.NonzeroNonisomorphism σ x y) :
    Relation.TransGen σ.irreducibleEdge x y := by
  obtain ⟨f, hfzero, hfiso⟩ := h
  have hfrad : QuotientSubmoduleEquidistribution.CategoricalRadical.IsRadicalMorphism f := by
...
theorem hasAcyclicNonzeroNonisomorphisms_of_irreducibleEdge_acyclic
  ...
  have lift : ∀ {a b : ι},
      Relation.TransGen
          (QuotientSubmoduleEquidistribution.RepresentationDirected.NonzeroNonisomorphism σ)
          a b →
...
def finiteDimensionalFgNilpotentRadicalData :
  ...
  let G :=
    QuotientSubmoduleEquidistribution.AuslanderEquivalence.FiniteTypeGenerator.additiveGenerator σ
  letI : IsArtinianRing (End G) :=
    QuotientSubmoduleEquidistribution.IndecomposableSkeleton.LegalQuotientDeletionChain.isArtinianRing_skeletonAuslanderAlgebra
      (K := K) σ
  exact
    QuotientSubmoduleEquidistribution.CategoricalRadical.nilpotentRadicalDataOfArtinianGenerator
      G
      (QuotientSubmoduleEquidistribution.AuslanderEquivalence.FiniteTypeGenerator.additiveGenerator_isFiniteAddGenerator
        σ)
...
theorem finiteDimensional_hasAcyclicNonzeroNonisomorphisms_of_irreducibleEdge_acyclic
    (hacyclic : ∀ x : ι,
      ¬ Relation.TransGen σ.irreducibleEdge x x) :
    QuotientSubmoduleEquidistribution.RepresentationDirected.HasAcyclicNonzeroNonisomorphisms σ :=
