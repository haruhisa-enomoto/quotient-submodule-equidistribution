...
theorem groundFinrank_orientation_of_hasIrreducibleMorphism
  ...
  rcases
      QuotientSubmoduleEquidistribution.RepresentationDirected.finrank_orientation_of_isIrreducibleMorphism
        (K := K) hf with h | h
  · exact Or.inl h.1
  · exact Or.inr h.1
...
theorem not_injective_of_hasIrreducibleMorphism_of_groundFinrank_lt
  ...
  have hdim' : Module.finrank K (σ.obj x) < Module.finrank K (σ.obj y) := by
    simpa [QuotientSubmoduleEquidistribution.RepresentationDirected.groundFinrank] using hdim
  letI : Mono f :=
...
theorem no_increasing_transitiveTriangle :
    ¬ ∃ T : TransitiveTriangle σ,
      QuotientSubmoduleEquidistribution.RepresentationDirected.groundFinrank
        ...
        QuotientSubmoduleEquidistribution.RepresentationDirected.groundFinrank
          (K := K) (σ.obj T.b) := by
  apply QuotientSubmoduleEquidistribution.RepresentationDirected.not_exists_of_finite_strict_ascent
