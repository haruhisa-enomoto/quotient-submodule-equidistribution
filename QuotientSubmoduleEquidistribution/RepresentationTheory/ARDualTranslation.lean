...
theorem backward_arTranslation_eq_inverse
  ...
  have hgAS : QuotientSubmoduleEquidistribution.IsLeftAlmostSplit g := by
    apply QuotientSubmoduleEquidistribution.IsLeftAlmostSplit.precomp_iso
      (D.backward.objIso q.1).symm
    exact A.rightAlmostSplit.map_op_equivalence
      D.backward.categoryEquiv
  have hgMin : QuotientSubmoduleEquidistribution.IsLeftMinimal g := by
    apply QuotientSubmoduleEquidistribution.IsLeftMinimal.precomp_iso
      (D.backward.objIso q.1).symm
    exact A.rightMinimal.map_op_equivalence
      D.backward.categoryEquiv
  have hL_AS : QuotientSubmoduleEquidistribution.IsLeftAlmostSplit L.map := L.leftAlmostSplit
  have hL_Min : QuotientSubmoduleEquidistribution.IsLeftMinimal L.map := L.leftMinimal
  let cokernelComparison : cokernel g ≅ cokernel L.map :=
    Classical.choice
      (QuotientSubmoduleEquidistribution.nonempty_cokernelIso_of_leftAlmostSplit
        hgAS hgMin hL_AS hL_Min)
  let mappedKernelIso :
      cokernel g ≅
        D.backward.categoryEquiv.functor.obj
          (Opposite.op (kernel A.map)) :=
    QuotientSubmoduleEquidistribution.cokernelPrecompMapOpIsoKernel
