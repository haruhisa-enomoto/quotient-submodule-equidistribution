...
structure InfiniteIndecomposableFamily (Parameter : Type z) where
  ...
  indecomposable : ∀ a, QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R (obj a)
...
theorem false_of_bundled_finite_skeleton
    (F : InfiniteIndecomposableFamily (R := R) Parameter)
    (σ : QuotientSubmoduleEquidistribution.FiniteIndecomposableSkeleton.{u, v, u} R) :
    False := by
