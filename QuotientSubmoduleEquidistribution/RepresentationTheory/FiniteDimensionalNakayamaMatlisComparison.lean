import QuotientSubmoduleEquidistribution.RepresentationTheory.FiniteProjectiveNakayamaEquiv
...
theorem fieldNakayamaHomEquiv_projectiveNaturality
    ...
    fieldNakayamaHomEquiv (K := K) (R := R) P Y
        (a ≫ (QuotientSubmoduleEquidistribution.Contragredient.reverseDualFunctor K R).map
...
def finiteDimensionalTwoStepNakayamaMatlisComparison
    ...
    TwoStepNakayamaMatlisComparison (k := K) T
      (QuotientSubmoduleEquidistribution.ArtinDuality.ofFiniteDimensional K) K where
  equiv₀ Y :=
