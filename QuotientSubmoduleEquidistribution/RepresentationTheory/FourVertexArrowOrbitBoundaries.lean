...
theorem arrowChain_squareShift_boundary_balance
    ...
    let M := AR.arMeshRotationData σ
    (∑ p : {p : QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.Grid
          (M.arrowChainLength σ a ha + 2)
          (M.arrowChainLength σ c hc + 2) //
        QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.Incoming p},
      if M.arrowChainVertexAt σ a ha p.1.1 =
          M.arrowChainVertexAt σ c hc p.1.2 then 1 else 0) =
    ∑ p : {p : QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.Grid
          (M.arrowChainLength σ a ha + 2)
          (M.arrowChainLength σ c hc + 2) //
        QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.Outgoing p},
      if M.arrowChainVertexAt σ a ha p.1.1 =
          M.arrowChainVertexAt σ c hc p.1.2 then 1 else 0 := by
  let M := AR.arMeshRotationData σ
  apply QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.sum_equalityIndicators_incoming_eq_outgoing
  intro p hp
  have hp' :
      ¬ QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.Outgoing p := hp
  have hcoords :
      p.1.1 + 2 < M.arrowChainLength σ a ha + 2 ∧
        p.2.1 + 2 < M.arrowChainLength σ c hc + 2 := by
    simpa [QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.Outgoing,
      not_or, not_le] using hp'
  have hna : p.1.1 < M.arrowChainLength σ a ha := by omega
  have hnc : p.2.1 < M.arrowChainLength σ c hc := by omega
  let p₂ := QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.addTwo p hp
  have haNP : ¬ Projective
      (σ.obj (M.arrowChainVertexAt σ a ha p₂.1)) := by
    simpa [p₂, QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.addTwo] using
  ...
  have hta : (M.tau xa₂).1 = M.arrowChainVertexAt σ a ha p.1 := by
    simpa [xa₂, p₂,
      QuotientSubmoduleEquidistribution.SquareShiftBoundaryBalance.addTwo] using
...
abbrev ArrowChainFirstDiagonalCut
    ...
    (ha : Projective (σ.obj a.1.2)) :=
  let M := AR.arMeshRotationData σ
  QuotientSubmoduleEquidistribution.DiagonalSquareShiftCuts.FirstCut
...
abbrev ArrowChainLastDiagonalCut
    ...
    (ha : Projective (σ.obj a.1.2)) :=
  let M := AR.arMeshRotationData σ
  QuotientSubmoduleEquidistribution.DiagonalSquareShiftCuts.LastCut
    (M.arrowChainLength σ a ha + 1)
    (AR.arrowChainVertexNat σ a ha)
...
def arrowChainFirstDiagonalCutEquivLastDiagonalCut
  ...
  change 4 ≤ n at hlen
  apply QuotientSubmoduleEquidistribution.DiagonalSquareShiftCuts.firstCutEquivLastCut
    (L := n + 1) (by omega) x
  · intro i j hi hj
