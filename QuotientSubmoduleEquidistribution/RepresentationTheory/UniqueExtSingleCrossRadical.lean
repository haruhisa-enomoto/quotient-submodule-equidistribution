...
theorem uniqueCrossExtArrow_singleCrossRadicalData
    ...
    i ≠ j ∧
      ...
      Module.finrank K
          (QuotientSubmoduleEquidistribution.QuotientSurvival.jacobsonCotangentSubmodule
            (K := K) (B := B)) ≤ 1 ∧
      ∃ c : B,
        c ∈ Ring.jacobson B ∧
  ...
  obtain ⟨c, hcJ, hcL, hcR, hc2⟩ :=
    QuotientSubmoduleEquidistribution.CoordinateGabrielRealization.exists_extArrow_bicorner_cotangent_representative
      D tau a
  have hspan :
      QuotientSubmoduleEquidistribution.QuotientSurvival.JacobsonCotangentSpannedBy
        (K := K) (0 : B) c 0 :=
