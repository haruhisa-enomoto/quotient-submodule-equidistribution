...
def fourVertexHookData (Deleted : Finset ι)
    ...
    QuotientSubmoduleEquidistribution.FourVertexHooks.Data
...
theorem AdmissibleHook.overlap_of_ne
  ...
  have hover := QuotientSubmoduleEquidistribution.FourVertexHooks.Data.Hook.overlap_of_ne
    (G := G) (by
      rw [natCard_deletedLabel_compl Deleted]
      exact hcard) hQ
...
theorem AdmissibleHook.first_projective_of_consecutive
  ...
  have hboundary :=
    QuotientSubmoduleEquidistribution.FourVertexHooks.Data.Hook.first_mem_boundary_of_consecutive
      (G := G) (by
        rw [natCard_deletedLabel_compl Deleted]
        exact hcard) hQ
