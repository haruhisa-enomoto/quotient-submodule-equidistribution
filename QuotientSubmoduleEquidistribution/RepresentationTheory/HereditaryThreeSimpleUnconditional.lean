...
import QuotientSubmoduleEquidistribution.RepresentationTheory.BottomThreeUnconditionalEndpoint
...
import QuotientSubmoduleEquidistribution.RepresentationTheory.TwoSimpleCoreThreeUnconditional
...
namespace QuotientSubmoduleEquidistribution.HereditaryThreeSimpleUnconditional
...
theorem fullProfileEquality_of_extSourceTarget_injective
  ...
  letI : IsArtinianRing B.Carrierᵐᵒᵖ :=
    QuotientSubmoduleEquidistribution.isArtinianRing_op_of_finiteDimensional K B.Carrier
  let tau :=
  ...
  let hNoParallel : NoParallelExtSupport (K := K) B.skeleton :=
    QuotientSubmoduleEquidistribution.FiniteDimensionalNoParallelExt.noParallelExtSupport
      B.skeleton
  ...
  have hProjective :
      QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsProjectiveNakayamaSkeleton
        B.skeleton :=
  ...
  have hInjective :
      QuotientSubmoduleEquidistribution.SerialRingBridge.IsInjectiveNakayamaSkeleton B.skeleton :=
...
theorem moduleTop_isSimple_of_twoTargetDecomposition
    ...
    {ι : Type v} [Finite ι]
    (σ : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, v, u} A ι)
  ...
  letI : IsSimpleModule A (σ.obj r) :=
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.simple_iff_isSimpleModule_fg _).mp hr
...
theorem radicalMultiplicityBounds_of_simpleTop_twoTypeDecomposition
  ...
  have hrNonzero : 𝟙 (σ.obj r).obj ≠ 0 :=
    CategoryTheory.id_nonzero (σ.obj r).obj
  have hbounds :=
    QuotientSubmoduleEquidistribution.YonedaExtReflection.shortExact_twoTarget_target_finrank_le_one
      (σ.obj r).obj (σ.obj s).obj (σ.obj t).obj (σ.obj j).obj
      hrNonzero hsNonzero htNonzero
      SC.f SC.g SC.zero hSC' (σ.indecomposable j)
...
theorem projectiveCoverLength_lt_of_extArrow
    ...
    (σ : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, v, u} A ι)
    (hHereditary :
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.FaithfulCore.ConnectedSmallCore.FinitelyGeneratedLeftHereditary
        A)
    (a : ExtGabrielArrowIndex (K := K) σ) :
    σ.compositionLength
        (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveLabelOfSimple
  ...
  let hNoParallel : NoParallelExtSupport (K := K) σ :=
    QuotientSubmoduleEquidistribution.FiniteDimensionalNoParallelExt.noParallelExtSupport σ
  ...
  let p :
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.ProjectiveIndex σ :=
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveIndexEquivSimpleIndex
      σ).symm s
  have hpTop :
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveTopIndex
          σ p = s :=
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveIndexEquivSimpleIndex
      σ).apply_symm_apply s
  let eTopFG :
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveTop σ p ≅
        σ.obj s.1 :=
    QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveTopIso σ p ≪≫
  ...
  let E :=
    QuotientSubmoduleEquidistribution.ProjectiveRadicalExt.radicalHomLinearEquivExtOne
      (K := K) (R := A) (P := σ.obj p.1)
  ...
  have hComponentIso : Nonempty
      (σ.obj (labels i) ≅
        σ.obj
          (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveLabelOfSimple
            σ t)) :=
    QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projective_iso_of_indec_of_epi_to_simple
...
theorem hasIrreducibleMorphism_projectiveCovers_of_extArrow
  ...
  apply
    (σ.indecomposableRetract_projectiveBoundaryRadical_iff_irreducible
      (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveLabelOfSimple
        σ s)
      (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projective_projectiveLabelOfSimple
        σ s)
      (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveLabelOfSimple
        σ t)).mp
...
theorem no_extTransitiveTriangle
  ...
  exact
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.FiniteARTranslationData.no_irreducible_transitiveTriangle
...
theorem projectiveRadical_eq_bot_of_no_extArrow_source
  ...
  letI : Nontrivial J := by
    ...
    exact hJ
  obtain ⟨t, q, hq⟩ :=
    QuotientSubmoduleEquidistribution.ProjectiveRadicalExt.exists_surjective_to_chosen_simple
      σ (M := J)
  have hpTop :
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveTopIndex
          σ p = s :=
...
theorem simple_projective_of_projectiveCoverRadical_eq_bot
  ...
  let eTarget : σ.obj p.1 ≅ σ.obj v.1 :=
    eBot.symm ≪≫
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveTopIso
        σ p ≪≫
      eqToIso (congrArg (fun z : σ.SimpleIndex => σ.obj z.1) hpTop)
...
theorem forkSource_projectiveRadical_isSemisimple
  ...
  have hComponentSimple (i : Fin n) :
    ...
    let ti : σ.SimpleIndex :=
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveTopIndex σ pi
    let qRad : σ.obj pi.1 ⟶
        QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.projectiveTop σ pi :=
      FGModuleCat.ofHom (Module.jacobson A (σ.obj pi.1)).mkQ
    haveI : Epi qRad :=
      (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective qRad).mpr
...
theorem forkSource_projectiveRadical_hasTwoTargetConstituents
  ...
  have hLIndec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule A LFG :=
...
theorem moduleRadical_isSemisimple_of_threeSimpleFork
  ...
  classical
  obtain ⟨P⟩ :=
    QuotientSubmoduleEquidistribution.IndecomposableSkeleton.ProjectiveSimpleRank.inFac_projectiveLabels
      σ (σ.obj j)
  ...
  have hsurj : Function.Surjective P.map.hom.hom :=
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective P.map).mp
      inferInstance
  let radicalMap :
      Module.jacobson A (σ.sumOver P.index P.label) →ₗ[A]
        σ.moduleRadical j :=
    QuotientSubmoduleEquidistribution.ProjectiveRadicalExt.radicalMapOfSurjective P.map.hom.hom
...
theorem exists_forkSource_projectiveRadical_target_isCompl
  ...
  have hsurj : Function.Surjective f :=
    QuotientSubmoduleEquidistribution.SerialEndpointReduction.surjective_of_injective_of_length_le
...
theorem exists_epi_forkSourceProjective_of_ne_targets
  ...
  · let jSimple : σ.SimpleIndex := ⟨j, hjSimple⟩
    ...
    · have hjSourceVal :
      ...
      exact
        QuotientSubmoduleEquidistribution.ExtDegreeNakayamaReduction.exists_epi_projectiveLabelOfSimple_of_simpleTop
  ...
  · let Q : σ.SimpleQuotient j :=
    ...
    have hQSurj : Function.Surjective Q.map.hom.hom :=
      (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective Q.map).mp
        inferInstance
    ...
    let eTopRaw : σ.moduleTop j ≃ₗ[A] σ.obj Q.index :=
      QuotientSubmoduleEquidistribution.FamilyFourControl.moduleTopLinearEquivOfSurjectiveToSimple
        ...
        · by_cases hkr : k = r
          ...
          · let f : σ.obj p ⟶ σ.obj j := sourceMap j hjt hjr
            ...
            have hgSurj : Function.Surjective g.hom.hom :=
              (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_epi_iff_surjective g).mp
                inferInstance
            ...
            have hfKerLe :
              ...
              QuotientSubmoduleEquidistribution.FamilyFourControl.le_jacobson_of_ne_top_of_simple_top
                hTopP hfKerNeTop
            have hgKerLe :
...
theorem levelCount_four_eq_of_indec_natCard_le_six
  ...
  · rw [QuotientSubmoduleEquidistribution.SetClosure.levelCount_eq_zero_of_card_lt
        σ.qClosure hlt,
      QuotientSubmoduleEquidistribution.SetClosure.levelCount_eq_zero_of_card_lt
        σ.sClosure hlt]
  · have hcases :
    ...
    · calc
        ...
        _ = 1 := QuotientSubmoduleEquidistribution.SetClosure.levelCount_card_eq_one _
...
theorem fgProjective_of_mono_to_fgProjective_of_finitelyGeneratedLeftHereditary
  ...
  exact QuotientSubmoduleEquidistribution.RingelStable.fgProjective_of_moduleProjective
    X hXProjective
...
theorem finitelyGeneratedLeftHereditary_op
  ...
  have hInclusionMono : Mono inclusion :=
    (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.fg_mono_iff_injective inclusion).mpr
      T.subtype_injective
  let E := QuotientSubmoduleEquidistribution.Contragredient.reverseDualityEquivalence K A
  let dualMap :
  ...
  exact QuotientSubmoduleEquidistribution.RingelStable.moduleProjective_of_fgProjective
    X hXProjective
...
theorem levelCount_four_eq_of_threeSimples_of_finitelyGeneratedLeftHereditary
    ...
    [IsNoetherianRing A] [IsNoetherianRing Aᵐᵒᵖ]
    {ι : Type (u + 1)} [Finite ι]
    (σ : QuotientSubmoduleEquidistribution.IndecomposableSkeleton.{u, u + 1, u} A ι)
    (hHereditary :
      QuotientSubmoduleEquidistribution.IndecomposableSkeleton.FaithfulCore.ConnectedSmallCore.FinitelyGeneratedLeftHereditary
        A)
    (hThree : Nat.card σ.SimpleIndex = 3) :
  ...
  · by_cases hTarget : Function.Injective
    ...
    · let τ :=
      ...
      let D :=
        QuotientSubmoduleEquidistribution.Contragredient.alignedBiduality K A σ τ
      letI : Finite
          (QuotientSubmoduleEquidistribution.CanonicalIndecomposableIndex.{u, u} Aᵐᵒᵖ) :=
        D.forward.labelEquiv.finite_iff.mp inferInstance
      let hNoParallelTau : NoParallelExtSupport (K := K) τ :=
        QuotientSubmoduleEquidistribution.FiniteDimensionalNoParallelExt.noParallelExtSupport τ
      ...
      have hTauNotSource : ¬ Function.Injective
        ...
        apply hTarget
        exact
          QuotientSubmoduleEquidistribution.ExtDualTransport.AlignedAntiEquivalence.extTarget_injective_of_dualExtSource_injective
      ...
      have hQLevel :
        ...
        rw [← QuotientSubmoduleEquidistribution.SetClosure.levelPolynomial_coeff,
          hQToS, QuotientSubmoduleEquidistribution.SetClosure.levelPolynomial_coeff]
      have hSLevel :
          σ.sClosure.levelCount 4 = τ.qClosure.levelCount 4 := by
        rw [← QuotientSubmoduleEquidistribution.SetClosure.levelPolynomial_coeff,
          hSToQ, QuotientSubmoduleEquidistribution.SetClosure.levelPolynomial_coeff]
...
theorem connectedSmallCore_three_four_unconditional
  ...
  · exact Or.inr
      (QuotientSubmoduleEquidistribution.BottomLevels.FiniteDimensionalRecurrence.AlgebraNode.levelCount_three_eq
        K B)
  · rcases subsingleton_or_nontrivial B.Carrier with hzero | hnonzero
    ...
    · letI : Nontrivial B.Carrier := hnonzero
      ...
      · by_cases hCoreThree : coreSize K B = 3
        · exact
            QuotientSubmoduleEquidistribution.BottomLevels.FiniteDimensionalRecurrence.twoSimpleCoreThree_four
...
end QuotientSubmoduleEquidistribution.HereditaryThreeSimpleUnconditional
