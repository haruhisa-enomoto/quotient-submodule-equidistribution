...
theorem unique_index_of_middle_indecomposable
    ...
    (hA : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R A.middle) :
    ...
    let ePi : A.middle ≃ₗ[R] ((t : A.index) → σ.obj (A.label t)) :=
      (FGModuleCat.isoToLinearEquiv A.decomposition).trans
        (FGModuleCat.isoToLinearEquiv
          (QuotientSubmoduleEquidistribution.IndecomposableSkeleton.biproductIsoPiFG Y))
...
def HasIndecomposableAlmostSplitMiddle : Prop :=
  ∀ x : σ.NonprojectiveLabel,
    QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R (D.chosenRightAR σ x).middle
...
theorem finiteDimensional_hasProjectiveInjectiveMiddle_of_indec
  ...
  have hAₓindec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R Aₓ.middle := by
  ...
  · by_contra hyP
    ...
    have hAᵧindec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R Aᵧ.middle := by
  ...
  · by_contra hyI
    ...
    have hAwIndec : QuotientSubmoduleEquidistribution.Foundation.IsIndecomposableModule R Aw.middle := by
...
theorem projectiveInjectiveMiddle_levelPolynomial_eq
  ...
  letI : Fintype {Deleted : Set ι // M.QGood Deleted} :=
    Fintype.ofFinite _
  have hq := QuotientSubmoduleEquidistribution.SetClosure.levelPolynomial_eq_sum_stat
    σ.qClosure qParam
  ...
  have hs := QuotientSubmoduleEquidistribution.SetClosure.levelPolynomial_eq_sum_stat
    σ.sClosure sParam
    (fun Deleted : {Deleted : Set ι // M.QGood Deleted} ↦
      Nat.card ι - Deleted.1.ncard) (by
        intro Deleted
