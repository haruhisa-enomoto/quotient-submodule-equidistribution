...
namespace QuotientSubmoduleEquidistribution.DualFixedSocleTransport
...
theorem isUniserialModule_of_image
  ...
  intro U V
  let e :=
    QuotientSubmoduleEquidistribution.LengthTwoGabrielBridge.IndecomposableSkeleton.AlignedAntiEquivalence.moduleSubobjectOrderIso
      σ τ D i
...
def fixedSocleChainDataOfFixedTop
    ...
    (Q : QuotientSubmoduleEquidistribution.NakayamaModuleChains.FixedTopChainData τ c) :
    QuotientSubmoduleEquidistribution.NakayamaModuleChains.FixedSocleChainData σ c where
...
def simpleIndexEquiv : σ.SimpleIndex ≃ τ.SimpleIndex where
  toFun i := ⟨D.labelEquiv i.1, by
    ...
    rw [QuotientSubmoduleEquidistribution.LengthTwoGabrielBridge.IndecomposableSkeleton.AlignedAntiEquivalence.compositionLength_eq
      σ τ D]
...
theorem levelPolynomial_eq_of_alignedBiduality
  ...
  let Qσ :=
    QuotientSubmoduleEquidistribution.NakayamaFixedTopChains.fixedTopChainData σ₀ hσ
  let Qτ :=
    QuotientSubmoduleEquidistribution.NakayamaFixedTopChains.fixedTopChainData τ₀ hτ
  let Sσ := fixedSocleChainDataOfFixedTop σ₀ τ₀ B.forward Qτ
  apply
    QuotientSubmoduleEquidistribution.NakayamaModuleChains.quotient_levelPolynomial_eq_submodule_of_fixedChains
      σ₀ Qσ Sσ (simpleIndexEquiv σ₀ τ₀ B.forward)
  · intro p
    exact
      QuotientSubmoduleEquidistribution.NakayamaFixedTopChains.fixedTopLabel_compositionLength
        σ₀ hσ p
  · intro p
    change
      σ₀.compositionLength
...
end QuotientSubmoduleEquidistribution.DualFixedSocleTransport
