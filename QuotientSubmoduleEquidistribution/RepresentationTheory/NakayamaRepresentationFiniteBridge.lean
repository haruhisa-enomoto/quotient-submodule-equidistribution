...
namespace QuotientSubmoduleEquidistribution.NakayamaRepresentationFiniteBridge
...
theorem all_indec_uniserial_of_simpleTops_of_projectiveUniserial
    (hTop :
      QuotientSubmoduleEquidistribution.ExtDegreeNakayamaReduction.AllIndecomposableTopsSimple σ)
    (hProjective :
      QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsProjectiveNakayamaSkeleton σ) :
    QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsNakayamaSkeleton σ := by
  intro i
  obtain ⟨p, hpProjective, f, hf⟩ :=
    exists_epi_from_indec_projective_of_simpleTop σ i (hTop i)
  letI : Epi f := hf
  have hP : QuotientSubmoduleEquidistribution.IsUniserialModule R (σ.obj p) :=
...
theorem label_eq_of_epi_from_uniserial_of_compositionLength_eq
    {p i j : ι}
    (hP : QuotientSubmoduleEquidistribution.IsUniserialModule R (σ.obj p))
  ...
  have hker :
    ...
    QuotientSubmoduleEquidistribution.NakayamaProjectiveQuotients.ker_eq_of_epi_of_length_eq_of_isUniserial
...
theorem finite_of_all_indec_uniserial
    (hNakayama :
      QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsNakayamaSkeleton σ) :
    Finite ι := by
  exact finite_of_simpleTops_of_projectiveUniserial σ
    (QuotientSubmoduleEquidistribution.ExtDegreeNakayamaReduction.simpleTops_of_isNakayamaSkeleton
      σ hNakayama)
    (QuotientSubmoduleEquidistribution.LocalNakayamaBranch.isProjectiveNakayamaSkeleton_of_isNakayamaSkeleton
      σ hNakayama)
...
theorem rightRepresentationFinite_of_all_indec_uniserial
    ...
    (hNakayama :
      ...
      let σA :=
        QuotientSubmoduleEquidistribution.rightIndecomposableSkeleton.{z, z, z} K A
      QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsNakayamaSkeleton σA) :
    QuotientSubmoduleEquidistribution.IsRightRepresentationFinite.{z, z, z} K A := by
  letI : IsNoetherianRing Aᵐᵒᵖ :=
    QuotientSubmoduleEquidistribution.isNoetherianRing_op_of_finiteDimensional K A
  let σA := QuotientSubmoduleEquidistribution.rightIndecomposableSkeleton.{z, z, z} K A
...
theorem rightEquidistribution_of_all_indec_uniserial
    ...
    (hNakayama :
      ...
      let σA :=
        QuotientSubmoduleEquidistribution.rightIndecomposableSkeleton.{z, z, z} K A
      QuotientSubmoduleEquidistribution.LocalNakayamaBranch.IsNakayamaSkeleton σA) :
    ∃ hA : QuotientSubmoduleEquidistribution.IsRightRepresentationFinite.{z, z, z} K A,
      QuotientSubmoduleEquidistribution.RightQuotientSubmoduleEquidistribution K A hA := by
  let hA : QuotientSubmoduleEquidistribution.IsRightRepresentationFinite.{z, z, z} K A :=
    rightRepresentationFinite_of_all_indec_uniserial K A hNakayama
  refine ⟨hA, ?_⟩
  apply
    QuotientSubmoduleEquidistribution.DualFixedSocleTransport.rightEquidistribution_of_all_indec_uniserial
      K A hA
  exact hNakayama
...
theorem rightEquidistribution_of_simpleTops_of_projectiveUniserial
    ...
    (hTop :
      ...
      let σA :=
        QuotientSubmoduleEquidistribution.rightIndecomposableSkeleton.{z, z, z} K A
      QuotientSubmoduleEquidistribution.ExtDegreeNakayamaReduction.AllIndecomposableTopsSimple σA)
    (hProjective :
      letI : IsNoetherianRing Aᵐᵒᵖ :=
        QuotientSubmoduleEquidistribution.isNoetherianRing_op_of_finiteDimensional K A
      let σA :=
...
end QuotientSubmoduleEquidistribution.NakayamaRepresentationFiniteBridge
