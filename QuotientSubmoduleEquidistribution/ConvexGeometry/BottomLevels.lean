...
def ClosedSingletons (c : QuotientSubmoduleEquidistribution.SetClosure E) :=
  {x : E // c.IsClosed {x}}
...
-/
def closedSingletonEquivLevelOne (c : QuotientSubmoduleEquidistribution.SetClosure E) :
    ClosedSingletons c ≃
...
theorem levelCount_one_eq_natCard_closedSingletons
    [Finite E] (c : QuotientSubmoduleEquidistribution.SetClosure E) :
