...
public import QuotientSubmoduleEquidistribution.Foundation.RepresentationTheory.Quiver.Acyclic.Basic
public import QuotientSubmoduleEquidistribution.Foundation.RepresentationTheory.Quiver.Representation.Projective.Basic
...
`QuotientSubmoduleEquidistribution.Foundation.finrank_hom_indecProjRep_indecProjRep` computes collapses to `1` on the endomorphisms of
`Pᵢ`: the projective `Pᵢ` is a brick.

## Main results

* `QuotientSubmoduleEquidistribution.Foundation.finrank_end_indecProjRep_of_isAcyclic`: `dim End(Pᵢ) = 1` over an acyclic quiver.
