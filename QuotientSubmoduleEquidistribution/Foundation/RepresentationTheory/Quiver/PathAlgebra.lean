...
* `QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath Q`: the total space `Σ a b, Path a b` of the paths of `Q`, the index
  type of the path basis.
* `QuotientSubmoduleEquidistribution.Foundation.Quiver.TotalPath.mul?`: concatenation of two indexed paths, `none` when they are not
  composable.
* `QuotientSubmoduleEquidistribution.Foundation.pathAlgebra k Q`: the path algebra, with `QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single` its basis
  ...
  `1 = ∑ᵥ eᵥ` exist.
* `QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.vertexIdempotent`: the idempotent `eᵥ` given by the trivial path at `v`.
* `QuotientSubmoduleEquidistribution.Foundation.pathAlgebraBasis`: the paths of `Q` as a `k`-basis of `kQ`.
* `QuotientSubmoduleEquidistribution.Foundation.Quiver.OneLoop`: the quiver with one vertex and one loop.

## Main results

* `QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.single_mul_single`: the defining product of two basis paths.
* `QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.one_def`: the vertex idempotents sum to `1`; they are
  orthogonal by `QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.vertexIdempotent_mul_vertexIdempotent_of_ne`.
* `QuotientSubmoduleEquidistribution.Foundation.module_finite_pathAlgebra` and `QuotientSubmoduleEquidistribution.Foundation.finrank_pathAlgebra`: `kQ` is a free module of
  rank the number of paths of `Q`. The specialization to a finite acyclic quiver, whose paths are
  finite, is `QuotientSubmoduleEquidistribution.Foundation.finiteDimensional_pathAlgebra_of_isAcyclic` in
  `QuotientSubmoduleEquidistribution.Foundation.RepresentationTheory.Quiver.Acyclic.PathAlgebra`.
* `QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.adjoin_vertexIdempotents_union_arrows`: the vertex idempotents and arrows
  generate the path algebra.
* `QuotientSubmoduleEquidistribution.Foundation.PathAlgebra.oneLoopAlgEquiv`: the one-loop path algebra is the additive monoid algebra
  on `ℕ`; `QuotientSubmoduleEquidistribution.Foundation.not_finiteDimensional_pathAlgebra_oneLoop` records that it is
