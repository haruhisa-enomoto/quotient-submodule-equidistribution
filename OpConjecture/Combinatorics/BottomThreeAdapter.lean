...
namespace OpConjecture.BottomLevels.BottomThreeAdapter
...
def ClosedLevelThree (c : OpConjecture.SetClosure X) :=
...
theorem levelCount_three_eq_natCard_closedLevelThree
    [Finite X] (c : OpConjecture.SetClosure X) :
...
variable [Finite X] [Fintype V] [Fintype E] [Fintype U]
  {c : OpConjecture.SetClosure X} {source target : E → V}
...
structure SwappedPairClassification
    (qClosure sClosure : OpConjecture.SetClosure X)
    [Fintype V] [Fintype E] [Fintype U]
    (source target : E → V) where
  qClassification :
...
end OpConjecture.BottomLevels.BottomThreeAdapter
