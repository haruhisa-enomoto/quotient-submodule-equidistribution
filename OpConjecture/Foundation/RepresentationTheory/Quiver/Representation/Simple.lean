...
* `OpConjecture.Foundation.simpleRep k Q i`: the vertex simple representation `Sᵢ`.

## Main results

* `OpConjecture.Foundation.simpleRep_simple`: `Sᵢ` is a simple object of `OpConjecture.Foundation.QuiverRep k Q`.
* `OpConjecture.Foundation.hom_simpleRep_eq_zero_iff` and `OpConjecture.Foundation.simpleRep_hom_eq_zero_iff`: a morphism into or
  out of `Sᵢ` is detected by its component at `i`.
* `OpConjecture.Foundation.dimVector_simpleRep`: the dimension vector of `Sᵢ` is `Pi.single i 1`.
* `OpConjecture.Foundation.not_nonempty_simpleRep_iso`: vertex simples at distinct vertices are not isomorphic.
...
object of `OpConjecture.Foundation.QuiverRep k Q`. -/
