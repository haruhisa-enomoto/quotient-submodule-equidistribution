...
public import OpConjecture.Foundation.RepresentationTheory.Quiver.Representation.DimensionVector
...
* `OpConjecture.Foundation.indecProjRep k Q i`: the representation `Pᵢ`.
* `OpConjecture.Foundation.indecProjRepBasis`: the paths `i → j` as a `k`-basis of `(Pᵢ)_j`.
* `OpConjecture.Foundation.indecProjRepHom`: the morphism `Pᵢ ⟶ M` determined by an element of `M` at `i`.

## Main results

* `OpConjecture.Foundation.indecProjRepHomEquiv`: `Pᵢ` **represents evaluation at `i`**, by the `k`-linear
...
* `OpConjecture.Foundation.projective_indecProjRep`: `Pᵢ` is a projective object of `OpConjecture.Foundation.QuiverRep k Q`.
* `OpConjecture.Foundation.dimVector_indecProjRep`: the dimension vector of `Pᵢ` counts the paths out of `i`, and
  `OpConjecture.Foundation.not_isZero_indecProjRep`: `Pᵢ` is nonzero.
* `OpConjecture.Foundation.finrank_hom_indecProjRep_indecProjRep`: `dim Hom(Pᵢ, Pⱼ)` is the number of paths
  `j → i`, the path-counting form of the Cartan matrix of a path algebra. Its acyclic consequence,
  that `Pᵢ` is then a brick, is in
  `OpConjecture.Foundation.RepresentationTheory.Quiver.Representation.Projective.Acyclic`.
...
`OpConjecture.Foundation.RepresentationTheory.Quiver.Representation.Simple` is built on `k` itself, so the two
