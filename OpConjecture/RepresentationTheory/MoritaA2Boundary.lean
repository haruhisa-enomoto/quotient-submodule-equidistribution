...
def factorNode [FiniteDimensional K R] [Finite iota]
  ...
  letI factorOppositeNoetherian :
    ...
    OpConjecture.isNoetherianRing_op_of_finiteDimensional
      K (Quotient.Factor I)
  let inflation :=
    OpConjecture.QuotientSkeletonAlignment.artinInflationData sigma I
  exact
...
theorem factorIndecomposableCount_eq_two_of_oneSimple
  ...
  have hOneB :
      OpConjecture.LocalNakayamaBranch.OneSimple B.skeleton := by
    simpa [OpConjecture.LocalNakayamaBranch.OneSimple, B] using hOne
  have hNoParallel :
  ...
  have hcard : Nat.card B.Index = 2 :=
    OpConjecture.OneSimpleCoreSaturation.natCard_eq_two_of_coreTwo_of_submoduleCore_eq_univ
...
theorem FiberData.ofOneSimple
  ...
  have hqLevel : IsEmpty (ArtinQLevel tau 3) :=
    OpConjecture.OneSimpleCoreSaturation.isEmpty_qLevel_three_of_natCard_eq_two
      tau hcard
  have hsLevel : IsEmpty (ArtinSLevel tau 3) :=
    OpConjecture.OneSimpleCoreSaturation.isEmpty_sLevel_three_of_natCard_eq_two
      tau hcard
