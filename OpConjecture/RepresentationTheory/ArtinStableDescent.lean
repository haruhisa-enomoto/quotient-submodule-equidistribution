...
instance reverseDualityFunctor_additive :
    (OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.Additive where
  map_add {X Y} f g := by
...
def factorsThroughInjective_reverseDual_op
    ...
    FactorsThroughInjective
      ((OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.map
        f.op) := by
  let E := OpConjecture.Contragredient.reverseDualityEquivalence K R
...
def factorsThroughInjective_reverseDual_difference
    ...
    FactorsThroughInjective
      ((OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.map f.op -
        (OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.map g.op) := by
  let E := OpConjecture.Contragredient.reverseDualityEquivalence K R
...
def artinAmbientStableFunctor :
  ...
  obj X :=
    (injectiveStableFunctor (R := R)).obj
      ((OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.obj
  ...
  map_id X := by
    change (injectiveStableFunctor (R := R)).map
      ((OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.map
        (𝟙 X.unop.as).op) = 𝟙 _
    rw [op_id,
      (OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.map_id,
      ...
      (injectiveStableFunctor (R := R)).map
        ...
        (injectiveStableFunctor (R := R)).map
          ((OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.map
            g.op)
    rw [op_comp,
      (OpConjecture.Contragredient.reverseDualityEquivalence K R).functor.map_comp,
