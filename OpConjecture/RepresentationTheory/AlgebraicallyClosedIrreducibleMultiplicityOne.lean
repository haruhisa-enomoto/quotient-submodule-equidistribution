...
theorem finrank_irreducibleHomSpace_eq_arTranslation_of_isAlgClosed
  ...
  rw [sigma.finrank_irreducibleHomSpace_eq_card_rightAROccurrence_of_isAlgClosed
    ...
    sigma.finrank_irreducibleHomSpace_eq_card_leftAROccurrence_of_isAlgClosed
        (K := K) (R := R)
        (OpConjecture.RepresentationDirected.leftAROnChosenRightMiddle
          sigma D z) x]
  rfl
...
theorem finrank_irreducibleHomSpace_le_one_of_finiteARTranslationData_of_isAlgClosed
  ...
  let dim : Iota → ℕ := fun i ↦
    OpConjecture.RepresentationDirected.groundFinrank (K := K) (sigma.obj i)
  ...
  have no_source_large : ¬ ∃ a b, Bad a b ∧ dim b < dim a := by
    apply OpConjecture.RepresentationDirected.no_large_source_pairs_of_strict_growth
      Bad dim
    intro a b hab hba
    ...
    have hbad' : Bad a' a := by
      ...
      exact hab
    have hgrowKernel :=
      OpConjecture.RepresentationDirected.rightAR_kernel_groundFinrank_gt_of_two_occurrences
        (K := K) sigma A hbnp t₁ t₂ hne ht₁ ht₂ hba
    have hkiso := OpConjecture.RepresentationDirected.groundFinrank_eq_of_iso
      (K := K) (D.arTranslationKernelIso sigma z)
    have hgrow : dim a < dim a' := by
      dsimp only [dim]
      change OpConjecture.RepresentationDirected.groundFinrank
          (K := K) (sigma.obj a) <
  ...
  have no_target_large : ¬ ∃ a b, Bad a b ∧ dim a < dim b := by
    apply OpConjecture.RepresentationDirected.no_large_target_pairs_of_strict_growth
      Bad dim
    ...
    have hkiso := OpConjecture.RepresentationDirected.groundFinrank_eq_of_iso
      (K := K) (D.arTranslationKernelIso sigma z)
    have hklt : OpConjecture.RepresentationDirected.groundFinrank
        (K := K) (kernel A.map : FGModuleCat.{u} R) < dim b := by
      rw [hkiso, htau]
      dsimp only [dim] at habdim
      exact habdim
    have hgrow :=
      OpConjecture.RepresentationDirected.rightAR_target_groundFinrank_gt_of_two_occurrences
  ...
  rcases OpConjecture.RepresentationDirected.finrank_orientation_of_isIrreducibleMorphism
      (K := K) hf with hxy | hyx
