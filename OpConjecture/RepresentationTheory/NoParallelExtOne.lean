...
namespace OpConjecture.NoParallelExtOne
...
theorem indecomposable_middle_of_extClass_ne_zero
    ...
    [IsArtinian R S.X₂]
    (hne : hS.extClass ≠ 0) :
    OpConjecture.Foundation.IsIndecomposableModule R S.X₂ := by
  ...
  apply
    OpConjecture.BottomTwoSimpleTop.isIndecomposableModule_of_length_eq_two_of_jacobson_ne_bot
      hlen
  intro hjac
...
theorem finrank_ext_one_le_one_of_finite_skeleton
  ...
  have hMindec (a : K) :
      OpConjecture.Foundation.IsIndecomposableModule R (M a) := by
...
theorem noParallelExtOne_of_finiteDimensional_of_finiteSkeleton
    ...
    ∀ {ι : Type v} [Finite ι]
      (σ : OpConjecture.IndecomposableSkeleton.{u, v, u} Aᵐᵒᵖ ι),
      OpConjecture.LoewyTwoRankCore.NoParallelExtOne σ K := by
  letI : IsNoetherianRing Aᵐᵒᵖ :=
    OpConjecture.isNoetherianRing_op_of_finiteDimensional K A
  ...
  letI : FiniteDimensional K (σ.obj s) :=
    OpConjecture.finiteDimensional_rightFGModule K A (σ.obj s)
  letI : FiniteDimensional K (σ.obj t) :=
    OpConjecture.finiteDimensional_rightFGModule K A (σ.obj t)
...
end OpConjecture.NoParallelExtOne
