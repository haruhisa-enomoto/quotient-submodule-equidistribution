...
namespace OpConjecture.CotangentExtBridge
...
def vertexTopLinearMap (i : I) :
    letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule i
    OpConjecture.Tsukamoto.principalRightIdeal (D.liftedCoordinate i) →ₗ[Bᵐᵒᵖ] K := by
  letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule i
  exact
...
def vertexTopMap (i : I) :
    OpConjecture.Tsukamoto.principalRightModule (D.liftedCoordinate i) ⟶
...
theorem vertexTopLinearMap_surjective (i : I) :
  ...
  let x : OpConjecture.Tsukamoto.principalRightIdeal e :=
    ⟨op (e * algebraMap K B y), by
      rw [show op (e * algebraMap K B y) =
        op (algebraMap K B y) * op e by simp]
      exact (OpConjecture.Tsukamoto.principalRightIdeal e).mul_mem_left _
...
theorem unop_mem_jacobson_of_mem_vertexTop_ker
  ...
  · have hfixedOp :
    ...
    rw [← hfixed, map_mul,
      OpConjecture.SplitBasicCoordinateSystem.QuotientCoordinateData.quotient_mk_liftedCoordinate]
    simp [OpConjecture.SplitBasicCoordinateSystem.QuotientCoordinateData.quotientCoordinate,
      hki]
...
theorem exists_nonzero_kernel_map_of_cross_cotangent
  ...
  let pₓ : OpConjecture.Tsukamoto.principalRightIdeal eᵢ :=
    ⟨op c, by
      rw [show op c = op (r * eⱼ) * op eᵢ by
        simp [c, eᵢ, eⱼ, mul_assoc]]
      exact (OpConjecture.Tsukamoto.principalRightIdeal eᵢ).mul_mem_left _
...
theorem vertexPresentation_projective_middle (i : I) :
  ...
  exact
    (IsProjective.iff_projective
      ...
      (OpConjecture.Tsukamoto.principalRightIdeal_projective
...
theorem linearMap_vertexProjective_coordinate_eq_zero
    {i j : I} (hij : i ≠ j) :
    letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule j
    ∀ g : OpConjecture.Tsukamoto.principalRightIdeal
  ...
  exact (Submodule.span_induction
    (p := fun y _ ↦ ∀ hy : y ∈
      OpConjecture.Tsukamoto.principalRightIdeal (D.liftedCoordinate i),
        g ⟨y, hy⟩ = 0)
    (fun y hy ↦ by
      rcases Set.mem_singleton_iff.mp hy with rfl
      intro hySpan
      let e := D.liftedCoordinate i
      let xₑ : OpConjecture.Tsukamoto.principalRightIdeal e :=
    ...
    (by
      intro hzero
      change g (0 : OpConjecture.Tsukamoto.principalRightIdeal
...
end OpConjecture.CotangentExtBridge
