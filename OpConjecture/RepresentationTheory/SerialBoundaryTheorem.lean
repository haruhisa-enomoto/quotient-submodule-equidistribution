...
theorem moduleInjective_of_fgInjective
  ...
  have hi : Mono i := by
    rw [OpConjecture.IndecomposableSkeleton.fg_mono_iff_injective]
...
theorem isInjectiveModuloIdeal_jacobson_pow_length_of_injective_boundary
    ...
    (hInjective :
      OpConjecture.SerialRingBridge.IsInjectiveNakayamaSkeleton tau)
    {U : Type u} [AddCommGroup U] [Module R U]
    (hUfinite : IsFiniteLength R U)
    (hUnontrivial : Nontrivial U)
    (hUuniserial : OpConjecture.IsUniserialModule R U) :
  ...
  have hQuniserial :
      OpConjecture.IsUniserialModule R (tau.obj q) :=
  ...
  obtain ⟨eIndex, hEInjective, f, hf⟩ :=
    OpConjecture.SerialRingBridge.exists_mono_to_indec_injective_of_simpleSocle
      (K := K) tau q hQsocle
  letI : Mono f := hf
  have hEuniserial :
      OpConjecture.IsUniserialModule R (tau.obj eIndex) :=
  ...
  have hgInjective : Function.Injective g.hom.hom :=
    (OpConjecture.IndecomposableSkeleton.fg_mono_iff_injective g).mp
      inferInstance
  ...
  have hAuniserial : OpConjecture.IsUniserialModule R A :=
