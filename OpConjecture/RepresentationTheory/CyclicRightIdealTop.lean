...
variable {K A : Type u} [Field K] [Ring A] [Algebra K A]
  ...
  (tau : OpConjecture.IndecomposableSkeleton.{u, v, u} Aᵐᵒᵖ kappa)
  {Arrow : Type w}
...
theorem cyclicRightIdeal_topIso_of_ne_zero_of_right_fixed
    ...
    Nonempty
      (FGModuleCat.of Aᵐᵒᵖ
          (OpConjecture.Tsukamoto.cyclicRightIdealFG c ⧸
            Module.jacobson Aᵐᵒᵖ
              (OpConjecture.Tsukamoto.cyclicRightIdealFG c)) ≅
        tau.obj i.1) := by
  let f := OpConjecture.Tsukamoto.principalRightIdealToCyclic
    (e := G.vertex i) (c := c)
  have hf : Function.Surjective f :=
    OpConjecture.Tsukamoto.principalRightIdealToCyclic_surjective hce
  let generator : OpConjecture.Tsukamoto.cyclicRightIdealFG c :=
  ...
  have hgenerator : generator ≠ 0 := by
    ...
    simpa using hval''
  letI : Nontrivial (OpConjecture.Tsukamoto.cyclicRightIdealFG c) :=
  ...
  have hVertexTopSimple :
      IsSimpleModule Aᵐᵒᵖ
        (OpConjecture.Tsukamoto.cyclicRightIdealFG (G.vertex i) ⧸
          Module.jacobson Aᵐᵒᵖ
            (OpConjecture.Tsukamoto.cyclicRightIdealFG (G.vertex i))) :=
  ...
  let eTop :=
    OpConjecture.Tsukamoto.topLinearEquivOfSurjectiveOfSimpleTop f hf
  let eTopFG :
      FGModuleCat.of Aᵐᵒᵖ
          (OpConjecture.Tsukamoto.cyclicRightIdealFG c ⧸
            Module.jacobson Aᵐᵒᵖ
