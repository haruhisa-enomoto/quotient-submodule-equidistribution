...
def LeftFlagTermPrincipalAlignment
    (hfinite :
      ∀ X : FGModuleCat.{wR} R, IsFiniteLength R X)
    (s : OpConjecture.SetClosure.ClosedFlag σ.sClosure) : Prop :=
...
theorem leftFlagTermPrincipalAlignment
  ...
  exact
    OpConjecture.IndecomposableSkeleton.LegalQuotientDeletionChain.transportedGenerated_carrier_iff_addPrincipal
...
structure MaximalLeftFlagAuslanderPackage
    (hfinite :
      ∀ X : FGModuleCat.{wR} R, IsFiniteLength R X)
    (s : OpConjecture.SetClosure.ClosedFlag σ.sClosure) where
  targetChain :
    SaturatedTotalTargetLeftRejectiveChain σ hfinite
      (skeletonAuslanderEquivalence σ)
  idealChain :
