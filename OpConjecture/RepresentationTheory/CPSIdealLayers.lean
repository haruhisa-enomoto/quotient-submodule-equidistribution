...
def rightIdealLayerRestrictedQuotientImageIsoForChain
    (H : IdempotentIdealChain A n) (i : Fin n) :
    rightIdealLayer H i ≅
      OpConjecture.Tsukamoto.restrictedQuotientImageRightModule
        (H.ideal i.succ) (H.ideal i.castSucc) :=
  OpConjecture.Tsukamoto.rightIdealLayerRestrictedQuotientImageIso
    (H.ideal i.succ) (H.ideal i.castSucc)
...
def FiniteAddHasMultiplicity
  ...
  ∀ Y : ModuleCat.{uA} S,
    OpConjecture.AuslanderEquivalence.finiteAddClosure X Y →
...
theorem finiteAddClosure_is_biproduct_moduleCat
  ...
  have hYaddFG :
      OpConjecture.AuslanderEquivalence.finiteAddClosure Xfg Yfg :=
    (OpConjecture.IndecomposableSkeleton.LegalQuotientDeletionChain.finiteAddClosure_fullSubcategory_iff
        (ModuleCat.isFG R) Xfg Yfg).mpr hYadd
  obtain ⟨m, ⟨e⟩⟩ :=
    OpConjecture.FiniteAddMultiplicity.finiteAddClosure_is_biproduct
...
def mapFiniteAddPresentation
    ...
    {X Y : C}
    (P : OpConjecture.AuslanderEquivalence.FiniteAddPresentation X Y) :
    OpConjecture.AuslanderEquivalence.FiniteAddPresentation
      (F.obj X) (F.obj Y) := by
...
theorem finiteAddClosure_map
    ...
    [PreservesFiniteBiproducts F]
    {X Y : C}
    (h : OpConjecture.AuslanderEquivalence.finiteAddClosure X Y) :
    OpConjecture.AuslanderEquivalence.finiteAddClosure
      (F.obj X) (F.obj Y) := by
  obtain ⟨P⟩ := h
...
theorem finiteAddClosure_target_of_iso
    ...
    [HasFiniteBiproducts C]
    {G X Y : C} (e : X ≅ Y)
    (h : OpConjecture.AuslanderEquivalence.finiteAddClosure G X) :
    OpConjecture.AuslanderEquivalence.finiteAddClosure G Y := by
  obtain ⟨P⟩ := h
  exact
    ⟨{
      n := P.n
...
theorem restrictedPrincipalIdeal_mem_add
    ...
    (hfinite :
      Module.Finite Bᵐᵒᵖ
        (OpConjecture.Tsukamoto.principalTwoSidedIdeal e))
    (hprojective :
      OpConjecture.Tsukamoto.IsRightProjectiveIdeal
        (OpConjecture.Tsukamoto.principalTwoSidedIdeal e)) :
    OpConjecture.AuslanderEquivalence.finiteAddClosure
      ((ModuleCat.restrictScalars (RingHom.op q)).obj
        (OpConjecture.Tsukamoto.principalRightModule e))
      ((ModuleCat.restrictScalars (RingHom.op q)).obj
        (ModuleCat.of Bᵐᵒᵖ
          (OpConjecture.Tsukamoto.principalTwoSidedIdeal e))) := by
  apply finiteAddClosure_map
    (ModuleCat.restrictScalars (RingHom.op q))
  exact
    OpConjecture.Tsukamoto.principalTwoSidedIdeal_mem_add_of_finite_projective
...
theorem QuotientPrincipalLayerRealization.layer_mem_add
    ...
    OpConjecture.AuslanderEquivalence.finiteAddClosure standard layer := by
  have hRestricted :=
    restrictedPrincipalIdeal_mem_add q e P.finiteIdeal P.projectiveIdeal
  have hLayer :
      OpConjecture.AuslanderEquivalence.finiteAddClosure
        ((ModuleCat.restrictScalars (RingHom.op q)).obj
          (OpConjecture.Tsukamoto.principalRightModule e)) layer :=
    finiteAddClosure_target_of_iso P.layerIso.symm hRestricted
  exact
    (OpConjecture.AuslanderEquivalence.finiteAddClosure_iff_of_iso
...
theorem indecomposable_of_surjective_from_indec_projective
    ...
    (hPindec : OpConjecture.Foundation.IsIndecomposableModule R P)
    [Nontrivial Y]
    (q : P →ₗ[R] Y) (hq : Function.Surjective q) :
    OpConjecture.Foundation.IsIndecomposableModule R Y := by
  letI : Nontrivial P := hPindec.nontrivial
  letI : IsLocalRing (Module.End R P) :=
    OpConjecture.Foundation.isLocalRing_end_of_isIndecomposable hPfinite hPindec
...
def rightStandardModuleChainDataOfFiniteLengthFiniteAdd
    ...
    (hstandardIndecomposable : ∀ i : Fin n,
      OpConjecture.Foundation.IsIndecomposableModule Aᵐᵒᵖ (D.standard i).object)
    (hadd : ∀ i : Fin n,
      OpConjecture.AuslanderEquivalence.finiteAddClosure
        ((D.standard i).object) (rightIdealLayer H i)) :
...
def principalRightIdealQuotientMap
    (q : A →+* B) (e : A) :
    OpConjecture.Tsukamoto.principalRightIdeal e →ₗ[Aᵐᵒᵖ]
      (ModuleCat.restrictScalars (RingHom.op q)).obj
        (OpConjecture.Tsukamoto.principalRightModule (q e)) where
  toFun x := by
    ...
    simpa [OpConjecture.Tsukamoto.principalRightIdeal,
...
def principalRightModuleQuotientMap
    ...
    OpConjecture.Tsukamoto.principalRightModule e ⟶
      (ModuleCat.restrictScalars (RingHom.op q)).obj
        (OpConjecture.Tsukamoto.principalRightModule (q e)) :=
...
theorem principalRightModuleQuotientMap_surjective
  ...
  have hxmem :
      xval ∈ OpConjecture.Tsukamoto.principalRightIdeal e := by
    have hgen :
        op e ∈ OpConjecture.Tsukamoto.principalRightIdeal e :=
      Ideal.subset_span (Set.mem_singleton (op e))
    have hmul :=
      (OpConjecture.Tsukamoto.principalRightIdeal e).mul_mem_left
        (op a) hgen
  ...
  rw [map_mul, ha]
  have hyfix :=
    OpConjecture.Tsukamoto.mul_op_eq_self_of_mem_principalRightIdeal
      (he.map q) y.property
...
def rightIdealToQuotientImageSemilinear
    ...
    I →ₛₗ[RingHom.op (Ideal.Quotient.mk J.asIdeal)]
      OpConjecture.Tsukamoto.quotientImage I J where
  toFun x :=
...
theorem rightIdealToQuotientImageSemilinear_surjective
  ...
  intro y
  obtain ⟨x, hx, hxy⟩ :=
    (OpConjecture.Tsukamoto.mem_twoSidedIdeal_map_iff_of_surjective
...
theorem quotientImage_finite
    ...
    Module.Finite (A ⧸ J.asIdeal)ᵐᵒᵖ
      (OpConjecture.Tsukamoto.quotientImage I J) := by
...
def selectedCoordinateProjectiveBiproductIsoPrincipalRightModule
    ...
    (⨁ fun j : T ↦ (coordinateProjective σ j.1).obj) ≅
      OpConjecture.Tsukamoto.principalRightModule
        (skeletonCoordinateProjector σ (fun k ↦ k ∈ T)) := by
  let P :=
    OpConjecture.AuslanderEquivalence.finiteProjectiveModules Γᵐᵒᵖ
  let I := P.ι
...
theorem range_le_coordinateProjectiveTrace_of_principalRightModule
    (T : Set κ) (r : κ)
    (g : OpConjecture.Tsukamoto.principalRightModule
  ...
  rintro y ⟨x, rfl⟩
  have hx := congrArg
    (fun f : OpConjecture.Tsukamoto.principalRightModule
          (skeletonCoordinateProjector σ (fun k ↦ k ∈ T)) ⟶
...
theorem map_coordinateProjective_mem_supportIdeal
    ...
    unop (f.hom x).1 ∈
      OpConjecture.Tsukamoto.principalTwoSidedIdeal
        (skeletonCoordinateProjector σ (fun k ↦ k ∈ T)) := by
  let ej := skeletonCoordinateProjector σ (fun k ↦ k = j)
  let eT := skeletonCoordinateProjector σ (fun k ↦ k ∈ T)
  let J := OpConjecture.Tsukamoto.principalTwoSidedIdeal eT
  have hejJ : ej ∈ J := by
    have hmono :
        OpConjecture.Tsukamoto.principalTwoSidedIdeal ej ≤ J :=
...
def coordinateSandwichedElement
  ...
  have hgen :
      op er ∈ OpConjecture.Tsukamoto.principalRightIdeal er :=
    Ideal.subset_span (Set.mem_singleton (op er))
  have hmul :=
    (OpConjecture.Tsukamoto.principalRightIdeal er).mul_mem_left
      (op (a * z * b)) hgen
  change op (er * (a * z * b)) ∈
    OpConjecture.Tsukamoto.principalRightIdeal er
...
theorem coordinateSandwichedElement_mem_trace_of_mem_supportIdeal
    (T : Set κ) (r : κ) (z : Γ)
    (hz : z ∈ OpConjecture.Tsukamoto.principalTwoSidedIdeal
      (skeletonCoordinateProjector σ (fun k ↦ k ∈ T))) :
    ∀ a b : Γ,
  ...
  | mem z hz =>
      ...
      have htarget : er * (er * a) = er * a := by
        rw [← mul_assoc, her.eq]
      let g : OpConjecture.Tsukamoto.principalRightModule eT ⟶
          (coordinateProjective σ r).obj :=
        ConcreteCategory.ofHom (C := ModuleCat Γᵐᵒᵖ)
          (OpConjecture.TsukamotoRadicalSandwichBridge.principalLinearBetween
            (source := eT) (target := er) htarget)
      have hg :=
        range_le_coordinateProjectiveTrace_of_principalRightModule
          σ T r g
      let xb : OpConjecture.Tsukamoto.principalRightIdeal eT :=
        ⟨op (eT * b), by
          have hgen :
              op eT ∈ OpConjecture.Tsukamoto.principalRightIdeal eT :=
            Ideal.subset_span (Set.mem_singleton (op eT))
          have hmul :=
            (OpConjecture.Tsukamoto.principalRightIdeal eT).mul_mem_left
              (op b) hgen
...
theorem deletionStep_quotientImage_eq_principalRemoved
    ...
    let J :=
      OpConjecture.Tsukamoto.principalTwoSidedIdeal
        (skeletonCoordinateProjector σ (d.support i.succ))
    let q := Ideal.Quotient.mk J.asIdeal
    OpConjecture.Tsukamoto.quotientImage I J =
      OpConjecture.Tsukamoto.principalTwoSidedIdeal
        (q (skeletonCoordinateProjector σ
          (fun j ↦ j = d.removed i))) := by
  dsimp only
  calc
    OpConjecture.Tsukamoto.quotientImage
        (OpConjecture.Tsukamoto.principalTwoSidedIdeal
          (skeletonCoordinateProjector σ (d.support i.castSucc)))
        (OpConjecture.Tsukamoto.principalTwoSidedIdeal
          (skeletonCoordinateProjector σ (d.support i.succ))) =
      OpConjecture.Tsukamoto.principalTwoSidedIdeal
        (Ideal.Quotient.mk
          (OpConjecture.Tsukamoto.principalTwoSidedIdeal
            (skeletonCoordinateProjector σ (d.support i.succ))).asIdeal
          (skeletonCoordinateProjector σ (d.support i.castSucc))) :=
      OpConjecture.TsukamotoRadicalSandwichBridge.map_principalTwoSidedIdeal
        _ _
    _ = _ := congrArg OpConjecture.Tsukamoto.principalTwoSidedIdeal
...
noncomputable def deletionCoordinateQuotientPrincipalLayerRealization
  ...
  have hJI : J ≤ I := by
    ...
    rw [d.step i] at hk
    exact hk.1
  have hJidem : OpConjecture.Tsukamoto.IsIdempotentIdeal J :=
    OpConjecture.Tsukamoto.principalTwoSidedIdeal_isIdempotent
  ...
  have hquotProjective :
      OpConjecture.Tsukamoto.IsRightProjectiveIdeal
        (OpConjecture.Tsukamoto.quotientImage I J) :=
    OpConjecture.Tsukamoto.quotientImage_isRightProjectiveIdeal
      J I hJI hJidem hupperProjective
  have hEq :
      OpConjecture.Tsukamoto.quotientImage I J =
        OpConjecture.Tsukamoto.principalTwoSidedIdeal (q er) :=
...
theorem deletionCoordinateStandardQuotient_indecomposable
    ...
    (d : OpConjecture.SetClosure.SaturatedSupportDeletionChain κ)
    (i : Fin (Fintype.card κ)) :
    OpConjecture.Foundation.IsIndecomposableModule Γᵐᵒᵖ
...
namespace OpConjecture.IndecomposableSkeleton.LegalQuotientDeletionChain

open OpConjecture.AuslanderEquivalence
open OpConjecture.AuslanderEquivalence.CoordinateIdempotent
open OpConjecture.Tsukamoto.StandardSemantics
...
variable {R : Type uR} [Ring R] [IsNoetherianRing R]
  {ι : Type uι}
  (σ : OpConjecture.IndecomposableSkeleton.{uR, uι, wR} R ι)
  [Fintype ι]
...
end OpConjecture.IndecomposableSkeleton.LegalQuotientDeletionChain
