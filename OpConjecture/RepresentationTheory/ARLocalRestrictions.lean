...
theorem groundFinrank_le_middle_of_occurrence
    ...
    OpConjecture.RepresentationDirected.groundFinrank
      ...
      OpConjecture.RepresentationDirected.groundFinrank
        (K := K) A.middle := by
  classical
  rw [OpConjecture.RepresentationDirected.groundFinrank_eq_sum_of_iso_sumOver
...
theorem groundFinrank_obj_pos (x : ι) :
    0 < OpConjecture.RepresentationDirected.groundFinrank
      (K := K) (σ.obj x) := by
...
theorem arTranslation_add_endpoint_groundFinrank_eq_middle
  ...
  have hdim :=
    OpConjecture.RepresentationDirected.finrank_add_finrank_eq_of_shortExact
      (K := K) (R := R) S hS
  have hk := OpConjecture.RepresentationDirected.groundFinrank_eq_of_iso
    (K := K) (AR.arTranslationKernelIso σ x)
  have hk' : OpConjecture.RepresentationDirected.groundFinrank
...
def boundaryTranslationChainData :
    OpConjecture.BoundaryTranslationChains.Data ι
...
theorem groundFinrank_lt_of_irreducible_to_projective
  ...
  · exact h.1
  · letI : Epi f := h.2
    exact (OpConjecture.RepresentationDirected.not_projective_target_of_isIrreducibleMorphism_of_epi
      hf hp).elim
...
theorem groundFinrank_lt_of_irreducible_from_injective
  ...
  rcases OpConjecture.RepresentationDirected.finrank_orientation_of_isIrreducibleMorphism
      (K := K) hf with h | h
  · letI : Mono f := h.2
    exact (OpConjecture.RepresentationDirected.not_injective_source_of_isIrreducibleMorphism_of_mono
      hf hi).elim
  · exact h.1
