import OpConjecture.Foundation.RepresentationTheory.Quiver.Acyclic.PathAlgebra
...
theorem isAcyclic {V : Type u} [Quiver.{v} V] :
    OpConjecture.Foundation.Quiver.IsAcyclic (Vertex V) := by
  intro a p
  cases p with
  | nil => rfl
  | cons p e =>
...
theorem finiteDimensional_pathAlgebra
    ...
    FiniteDimensional k (OpConjecture.Foundation.pathAlgebra k (Vertex V)) :=
  OpConjecture.Foundation.finiteDimensional_pathAlgebra_of_isAcyclic
