...
theorem isFaithfulSupport_of_regular_inSub
  ...
  have hrSelected : ∀ t (x : σ.obj (P.label t)), r • x = 0 :=
    fun t x ↦
      (OpConjecture.AnnihilatorInflation.mem_supportAnnihilator σ.obj).mp hr
        (P.label t) (P.mem t) x
  have hrSum :
  ...
  have hinjective : Function.Injective P.map.hom.hom :=
    (OpConjecture.IndecomposableSkeleton.fg_mono_iff_injective P.map).1 P.mono
  have : r = 0 := by
    apply hinjective
...
theorem closedFaithfulNormalForm_of_artinian
    ...
    ClosedFaithfulNormalForm σ
      ...
      (OpConjecture.AnnihilatorInflation.IsFaithfulSupport σ.obj) where
  quotient_iff := by
    intro C
    constructor
    · intro hC i hi
...
theorem faithfulQPair_eq_quotientCore
    ...
    (hCfaithful :
      OpConjecture.AnnihilatorInflation.IsFaithfulSupport σ.obj (C : Set ι))
  ...
  have hcoreNeZero : (quotientCore σ : Set ι).ncard ≠ 0 := by
    ...
    have htopbot : (⊤ : TwoSidedIdeal R) = ⊥ := by
      simpa [hcoreEmpty,
        OpConjecture.AnnihilatorInflation.IsFaithfulSupport] using
          hcoreFaithful
    have hone : (1 : R) = 0 := by
...
def qPairSigmaAnnihilatorEquiv :
    ArtinQPair σ ≃
      Σ I : TwoSidedIdeal R,
        OpConjecture.AnnihilatorInflation.Skeleton.InflationData.QAnnihilatorLevel
          σ I 2 :=
...
def sPairSigmaAnnihilatorEquiv :
    ArtinSPair σ ≃
      Σ I : TwoSidedIdeal R,
        OpConjecture.AnnihilatorInflation.Skeleton.InflationData.SAnnihilatorLevel
          σ I 2 :=
...
variable {L : TwoSidedIdeal R → Type vq}
  [factorNoetherian :
    ∀ I : TwoSidedIdeal R,
      IsNoetherianRing (OpConjecture.AnnihilatorInflation.Quotient.Factor I)]
  (τ : ∀ I : TwoSidedIdeal R,
    IndecomposableSkeleton.{u, vq, u}
      (OpConjecture.AnnihilatorInflation.Quotient.Factor I) (L I))
  (D : ∀ I : TwoSidedIdeal R,
    OpConjecture.AnnihilatorInflation.Skeleton.InflationData σ I (τ I))
  (normal : ∀ I : TwoSidedIdeal R,
    ClosedFaithfulNormalForm (τ I)
      (OpConjecture.AnnihilatorInflation.IsFaithfulSupport (τ I).obj)
      (OpConjecture.AnnihilatorInflation.IsFaithfulSupport (τ I).obj))
...
def fixedAnnihilatorPairEquiv (I : TwoSidedIdeal R) :
    OpConjecture.AnnihilatorInflation.Skeleton.InflationData.QAnnihilatorLevel
      ...
      OpConjecture.AnnihilatorInflation.Skeleton.InflationData.SAnnihilatorLevel
        σ I 2 :=
  (OpConjecture.AnnihilatorInflation.Skeleton.InflationData.faithfulQLevelEquiv
      σ I (τ I) (D I) 2).symm |>.trans
    (faithfulPairEquiv (τ I) (normal I) (coreEquiv I)) |>.trans
      (OpConjecture.AnnihilatorInflation.Skeleton.InflationData.faithfulSLevelEquiv
        σ I (τ I) (D I) 2)
...
theorem qPairCoreTwoIdealEquiv_ideal (C : ArtinQPair σ) :
    (qPairCoreTwoIdealEquiv σ τ D normal coreEquiv C).1 =
      OpConjecture.AnnihilatorInflation.supportAnnihilator σ.obj
...
theorem annihilatorPreservingPairEquiv_annihilator
  ...
  let I := OpConjecture.AnnihilatorInflation.supportAnnihilator σ.obj
    ((C.1 : σ.qClosure.Closeds) : Set ι)
  let X :
      OpConjecture.AnnihilatorInflation.Skeleton.InflationData.QAnnihilatorLevel
        σ I 2 :=
