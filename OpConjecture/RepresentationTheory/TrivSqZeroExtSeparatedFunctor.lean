...
namespace OpConjecture.TrivSqZeroExtSeparatedFunctor
...
instance separatedLayerFunctor_reflectsIsomorphisms :
  ...
  OpConjecture.CategoryTheory.IdempotentLifting.reflectsIsomorphisms_of_kernelSquareZero
...
def isoOfSeparatedLayerIso
    ...
    (e : (separatedLayerFunctor (S := S) (J := J)).obj X ≅
      (separatedLayerFunctor (S := S) (J := J)).obj Y) :
    X ≅ Y :=
  OpConjecture.CategoryTheory.IdempotentLifting.isoOfMapIsoOfKernelSquareZero
    (separatedLayerFunctor (S := S) (J := J))
    separatedLayerFunctor_kernelSquareZero e

end OpConjecture.TrivSqZeroExtSeparatedFunctor
