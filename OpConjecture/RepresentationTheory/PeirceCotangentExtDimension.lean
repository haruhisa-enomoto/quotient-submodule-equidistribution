...
def cornerDualKernelMap
    ...
    OpConjecture.CotangentExtBridge.CoordinateData.KernelToCoordinateLinearMap
      D i j := by
  letI : Module Bᵐᵒᵖ K :=
    OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimpleModule D j
  exact
    { toFun := fun x ↦
        phi (D.cornerCotangentElement i j (unop x.1.1)
          (OpConjecture.CotangentExtBridge.CoordinateData.unop_mem_jacobson_of_mem_vertexTop_ker
            D i x))
      map_add' := fun x y ↦ by
        rw [← map_add]
        congr 1
...
def cornerDualToKernelLinearMap (i j : Fin 2) :
    Module.Dual K (D.jacobsonCotangentCornerSubmodule i j) →ₗ[K]
      (OpConjecture.CotangentExtBridge.CoordinateData.vertexPresentation D i).X₁ ⟶
        OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D j where
  toFun phi :=
    OpConjecture.CotangentExtBridge.CoordinateData.kernelToCoordinateMorphism
...
theorem cornerDualKernelMap_vertexKernelElementOfJacobian
    ...
    D.cornerDualKernelMap i j phi
        (OpConjecture.CotangentExtBridge.CoordinateData.vertexKernelElementOfJacobian
          D i z hz) =
    ...
    { toFun := fun z ↦
      ...
      map_add' := fun x y ↦ by
        have hadd :
          ...
          apply Subtype.ext
          apply Subtype.ext
          simp [OpConjecture.CotangentExtBridge.CoordinateData.vertexKernelElementOfJacobian,
            mul_add]
        rw [hadd, map_add]
      map_smul' := fun c z ↦ by
        have hsmul :
          ...
          apply Subtype.ext
          apply Subtype.ext
          simp only [OpConjecture.CotangentExtBridge.CoordinateData.vertexKernelElementOfJacobian,
...
theorem ker_cornerCotangentPresentation_le_ker_kernelMapOnJacobian
  ...
  have hzero :=
    OpConjecture.CotangentExtBridge.CoordinateData.kernelMap_eq_zero_on_jacobson_sq
      D i j f hsq
  have hproject :
      OpConjecture.CotangentExtBridge.CoordinateData.vertexKernelElementOfJacobian
  ...
  simpa [OpConjecture.CotangentExtBridge.CoordinateData.coordinateCharacterOp]
    using hzero
...
noncomputable def cornerDualLinearEquivKernelMorphism (i j : Fin 2) :
    Module.Dual K (D.jacobsonCotangentCornerSubmodule i j) ≃ₗ[K]
      ((OpConjecture.CotangentExtBridge.CoordinateData.vertexPresentation
          D i).X₁ ⟶
        OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D j) :=
...
theorem vertexPresentation_restrict_zero
    ...
    ∀ g :
      ...
      (OpConjecture.CotangentExtBridge.CoordinateData.vertexPresentation
          D i).f ≫ g = 0 := by
  have hker :=
    OpConjecture.CoordinateGabrielRealization.CoordinateData.ker_vertexTopLinearMap_eq_jacobson
      D i
  letI : IsSimpleModule Bᵐᵒᵖ
      (OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D j) :=
    (simple_iff_isSimpleModule'
      (OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D j)).mp
        (OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple_simple
          D j)
  ...
  change g.hom x.1 = 0
  have hxRad :
      (x.1 : OpConjecture.Tsukamoto.principalRightIdeal
          (D.liftedCoordinate i)) ∈
...
noncomputable def kernelMorphismLinearEquivCoordinateExt
    ...
    ((OpConjecture.CotangentExtBridge.CoordinateData.vertexPresentation
        D i).X₁ ⟶
      OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D j) ≃ₗ[K]
      Ext
        (OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D i)
        (OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D j)
        1 := by
  let hS :=
    OpConjecture.CotangentExtBridge.CoordinateData.vertexPresentation_shortExact
      D i
  letI : Projective
      (OpConjecture.CotangentExtBridge.CoordinateData.vertexPresentation
        D i).X₂ :=
    OpConjecture.CotangentExtBridge.CoordinateData.vertexPresentation_projective_middle
      D i
  exact
    (Ext.linearEquiv₀ (R := K)).symm.trans
      (LinearEquiv.ofBijective
        (hS.extClass.precompOfLinear K
          (OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimple D j)
          (add_zero 1))
        (OpConjecture.ProjectiveRadicalExt.extClass_precomp_bijective_of_projective_middle_of_restrict_zero
...
noncomputable def coordinateExtOneLinearEquivAlignedSource
    ...
    Ext
      ...
      Ext
        ...
        1 := by
  let e :=
    OpConjecture.CotangentExtBridge.CoordinateData.coordinateSimpleIsoAligned
      D tau i
  exact
    { toFun := fun alpha ↦
