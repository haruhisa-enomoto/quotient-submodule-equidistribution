...
theorem natCard_lengthTwoIndex_eq_one
  ...
  let hcases (s : sigma.SimpleIndex) : s = short ∨ s = long := by
    ...
    · exact False.elim (hne (hsb.trans hlb.symm))
  let q : OpConjecture.NakayamaCombinatorics.ChainPoint
...
theorem natCard_extGabrielArrowIndex_eq_one
    ...
    (hNoParallel :
      OpConjecture.GabrielArrowBridge.NoParallelExtSupport
        (K := K) sigma) :
    Nat.card
        (OpConjecture.GabrielArrowBridge.ExtGabrielArrowIndex
          (K := K) sigma) = 1 := by
  rw [← OpConjecture.GabrielArrowBridge.LengthTwo.natCard_lengthTwoIndex_eq_extGabrielArrowIndex
    sigma hNoParallel]
...
theorem isNakayamaSkeleton_of_twoSimples_of_projectiveCount_eq_coreSize
    ...
    OpConjecture.LocalNakayamaBranch.IsNakayamaSkeleton B.skeleton := by
