...
namespace OpConjecture.LollipopConcrete.B1.ModuleLayer.SevenObjectAdapter
...
abbrev Label :=
  OpConjecture.BottomLevels.LollipopRelationTables.LivePathLabel
...
structure Classification where
  indecomposable :
    ∀ i, OpConjecture.Foundation.IsIndecomposableModule (B1Model K) (obj K i)
  eq_of_iso :
    ∀ {i j}, Nonempty (obj K i ≅ obj K j) → i = j
  exhaustive :
    ∀ M : FGModuleCat (B1Model K),
      OpConjecture.Foundation.IsIndecomposableModule (B1Model K) M →
...
def skeleton (C : Classification K) :
  ...
  decomposes := by
    intro M
    obtain ⟨n, Y, hY, ⟨e⟩⟩ :=
      OpConjecture.FiniteLengthDecomposition.exists_fin_biproduct_decomposition
...
def quotientOmissionOfRangeBound
    ...
    (hrange : ∀ i : Label, i ∈ S → ∀ f : obj K i ⟶ obj K j,
      LinearMap.range f.hom.hom ≤ bound) :
    OpConjecture.BottomLevels.LollipopTableCertificates.Quotient.OmissionCertificate
...
def quotientA_omission_S2 (C : Classification K) :
  ...
  OpConjecture.BottomLevels.LollipopTableCertificates.Quotient.omissionCertificateOfHomEqZero
...
def submoduleOmissionOfKernelWitness
    ...
    (hker : ∀ i : Label, i ∈ S → ∀ f : obj K j ⟶ obj K i,
      witness ≤ LinearMap.ker f.hom.hom) :
    OpConjecture.BottomLevels.LollipopTableCertificates.Submodule.OmissionCertificate
      (skeleton K C) S j where
  not_mem := hnot
  witness := witness
...
def actualLivePathCertificates
    ...
    (QCore : OpConjecture.BottomLevels.MinimalFaithfulCore.Data
      (skeleton K C).qClosure FaithfulQ)
    (SCore : OpConjecture.BottomLevels.MinimalFaithfulCore.Data
    ...
    OpConjecture.BottomLevels.LollipopRelationTables.LivePathCertificates
...
def actualTableData
    ...
    (quotient_core : (QCore.core : Set Label) = quotientCore)
    (submodule_core : (SCore.core : Set Label) = submoduleCore) :
    OpConjecture.BottomLevels.ConnectedSmallCore.LollipopTableData
      (fun _ : Unit ↦ Label)
...
end OpConjecture.LollipopConcrete.B1.ModuleLayer.SevenObjectAdapter
