...
namespace OpConjecture.FamilyFiveCommonTargetConstruction
...
open OpConjecture.YonedaExtReflection
...
theorem indecomposable_middle_of_twoTop_nonzero_rows
  ...
  have hJker : Module.jacobson R M = LinearMap.ker g.hom := by
    ...
    · letI : IsSemisimpleModule R M :=
      ...
      apply False.elim
      apply hclass
      apply OpConjecture.NoParallelExtOne.extClass_eq_zero_of_section
...
structure ConstructedTwoTopPreApexData
    ...
    {ι : Type u}
    (σ : OpConjecture.IndecomposableSkeleton.{u, u, u} R ι)
...
theorem exists_constructedTwoTopPreApexData_of_shortExact
    ...
    (hleft : extBiprodLeft hseq.extClass ≠ 0)
    (hright : extBiprodRight hseq.extClass ≠ 0)
    (hMindec : OpConjecture.Foundation.IsIndecomposableModule R M) :
  ...
  letI : Epi qLeft :=
    (OpConjecture.IndecomposableSkeleton.fg_epi_iff_surjective qLeft).mpr
      hqLeftSurj
  ...
  letI : Epi qRight :=
    (OpConjecture.IndecomposableSkeleton.fg_epi_iff_surjective qRight).mpr
      hqRightSurj
  ...
  have hquotients :
    ...
    have hqRawSurj : Function.Surjective qRawLinear := by
      exact (OpConjecture.IndecomposableSkeleton.fg_epi_iff_surjective
  ...
  have hnoLengthTwo :
    ...
    have hqSurj : Function.Surjective q.hom.hom :=
      (OpConjecture.IndecomposableSkeleton.fg_epi_iff_surjective q).mp
        inferInstance
...
theorem exists_constructedTwoTopPreApexData_of_commonTarget
    ...
    ∀ {ι : Type u} [Finite ι]
      ...
      e₁ ≠ e₂ →
      OpConjecture.GabrielArrowBridge.LengthTwo.target σ e₂ =
        OpConjecture.GabrielArrowBridge.LengthTwo.target σ e₁ →
      Nonempty
        (ConstructedTwoTopPreApexData σ
          (OpConjecture.GabrielArrowBridge.LengthTwo.source σ e₁)
          (OpConjecture.GabrielArrowBridge.LengthTwo.source σ e₂)
          (OpConjecture.GabrielArrowBridge.LengthTwo.target σ e₁)) := by
  letI : IsNoetherianRing Bᵐᵒᵖ :=
    OpConjecture.isNoetherianRing_op_of_finiteDimensional K B
  letI : IsArtinianRing Bᵐᵒᵖ :=
    OpConjecture.isArtinianRing_op_of_finiteDimensional K B
  ...
  classical
  let s := OpConjecture.GabrielArrowBridge.LengthTwo.source σ e₁
  let t := OpConjecture.GabrielArrowBridge.LengthTwo.source σ e₂
  let r := OpConjecture.GabrielArrowBridge.LengthTwo.target σ e₁
  let r₂ := OpConjecture.GabrielArrowBridge.LengthTwo.target σ e₂
  ...
  let U₂ : ModuleCat.{u} Bᵐᵒᵖ := (σ.obj r₂.1).obj
  have hnoParallel :=
    OpConjecture.GabrielArrowBridge.RightModules.noParallelExtSupport_of_finiteDimensional_of_finiteSkeleton
      K B σ
  have hst : s.1 ≠ t.1 := by
    intro hindex
    have hs : s = t := Subtype.ext hindex
    apply he₁e₂
    apply OpConjecture.GabrielArrowBridge.LengthTwo.toGabrielArrow_injective
      σ hnoParallel
    apply Subtype.ext
  ...
  let xi : Ext S U 1 :=
    OpConjecture.GabrielArrowBridge.LengthTwo.extensionClass σ e₁
  let eta : Ext T U 1 :=
    (OpConjecture.GabrielArrowBridge.LengthTwo.extensionClass σ e₂).comp
      (Ext.mk₀ bU) (add_zero 1)
  have hxi : xi ≠ 0 := by
    exact
      OpConjecture.GabrielArrowBridge.LengthTwo.extensionClass_ne_zero σ e₁
  have heta : eta ≠ 0 := by
    intro hzero
    apply
      OpConjecture.GabrielArrowBridge.LengthTwo.extensionClass_ne_zero σ e₂
  ...
  let M : FGModuleCat.{u} Bᵐᵒᵖ :=
    FGModuleCat.of Bᵐᵒᵖ
      (OpConjecture.NoParallelExtOne.PushoutExtension.middle p push)
  let SC :=
    OpConjecture.NoParallelExtOne.PushoutExtension.shortComplex p push
  let hseq : SC.ShortExact :=
    OpConjecture.NoParallelExtOne.PushoutExtension.shortExact p hp push
  ...
  letI : FiniteDimensional K (σ.obj s.1) :=
    OpConjecture.finiteDimensional_rightFGModule K B (σ.obj s.1)
  letI : FiniteDimensional K (σ.obj t.1) :=
    OpConjecture.finiteDimensional_rightFGModule K B (σ.obj t.1)
  letI : FiniteDimensional K (σ.obj r.1) :=
    OpConjecture.finiteDimensional_rightFGModule K B (σ.obj r.1)
  letI : FiniteDimensional K (S ⟶ S) :=
    OpConjecture.NoParallelExtOne.moduleFinite_moduleCatHom_of_finiteDimensional
      (K := K) (R := Bᵐᵒᵖ)
  ...
  have hMindec : OpConjecture.Foundation.IsIndecomposableModule Bᵐᵒᵖ M := by
      ...
      ∃ x : σ.TwoTopQuotientShapeIndex,
        Nonempty (σ.ActualTwoTopApexData x) ∧
          σ.simpleQuotientIndexSet x.1 =
            { (OpConjecture.GabrielArrowBridge.LengthTwo.source σ e₁).1,
              (OpConjecture.GabrielArrowBridge.LengthTwo.source σ e₂).1 } := by
  letI : IsNoetherianRing Bᵐᵒᵖ :=
    OpConjecture.isNoetherianRing_op_of_finiteDimensional K B
  letI : IsArtinianRing Bᵐᵒᵖ :=
    OpConjecture.isArtinianRing_op_of_finiteDimensional K B
...
theorem exists_actualTwoTopApexData_of_commonTargetPair
    ...
    ∀ {ι : Type u} [Finite ι]
      ...
      (p : σ.CommonTargetPair
        (OpConjecture.GabrielArrowBridge.LengthTwo.target σ)),
      ∃ x : σ.TwoTopQuotientShapeIndex,
        Nonempty (σ.ActualTwoTopApexData x) ∧
          σ.simpleQuotientIndexSet x.1 =
            σ.commonTargetPairSourceSet
              (OpConjecture.GabrielArrowBridge.LengthTwo.source σ)
              (OpConjecture.GabrielArrowBridge.LengthTwo.target σ) p := by
  letI : IsNoetherianRing Bᵐᵒᵖ :=
    OpConjecture.isNoetherianRing_op_of_finiteDimensional K B
  letI : IsArtinianRing Bᵐᵒᵖ :=
    OpConjecture.isArtinianRing_op_of_finiteDimensional K B
  ...
  have he : a.1 ≠ b.1 := by
    ...
    exact Subtype.ext h
  have ht :
      OpConjecture.GabrielArrowBridge.LengthTwo.target σ b.1 =
        OpConjecture.GabrielArrowBridge.LengthTwo.target σ a.1 :=
    b.2.trans a.2.symm
  obtain ⟨x, hactual, hsources⟩ :=
    exists_actualTwoTopApexData_of_commonTarget
      K B σ a.1 b.1 he ht
  refine ⟨x, hactual, hsources.trans ?_⟩
  unfold OpConjecture.IndecomposableSkeleton.commonTargetPairSourceSet
  rw [hpair]
  ext z
  simp [eq_comm]
...
theorem exists_constructedCommonTargetPairApexData
  ...
  obtain ⟨D⟩ :=
    exists_constructedTwoTopPreApexData_of_commonTarget
      K B σ a.1 b.1 he ht
  let s := OpConjecture.GabrielArrowBridge.LengthTwo.source σ a.1
  let t := OpConjecture.GabrielArrowBridge.LengthTwo.source σ b.1
  let r := OpConjecture.GabrielArrowBridge.LengthTwo.target σ a.1
  let x := D.toTwoTopQuotientShapeIndex σ s t r
  ...
  · refine (D.simpleQuotientIndexSet_eq σ s t r).trans ?_
    ...
    simp [s, t, eq_comm]
  · change
      (OpConjecture.GabrielArrowBridge.LengthTwo.target σ a.1).1 = p.1.1
...
def constructedCommonTargetPairApexData
    ...
    ConstructedCommonTargetPairApexData σ
      (OpConjecture.GabrielArrowBridge.LengthTwo.source σ)
      (OpConjecture.GabrielArrowBridge.LengthTwo.target σ) p :=
  Classical.choice
...
theorem commonTargetPair_eq_of_target_eq_of_sourceSet_eq
    (p q : σ.CommonTargetPair
      (OpConjecture.GabrielArrowBridge.LengthTwo.target σ))
    (hnoParallel :
      OpConjecture.GabrielArrowBridge.NoParallelExtSupport (K := K) σ)
    (ht : p.1 = q.1)
    (hsources :
      σ.commonTargetPairSourceSet
          (OpConjecture.GabrielArrowBridge.LengthTwo.source σ)
        ...
        σ.commonTargetPairSourceSet
          (OpConjecture.GabrielArrowBridge.LengthTwo.source σ)
          (OpConjecture.GabrielArrowBridge.LengthTwo.target σ) q) :
    p = q := by
  classical
  rcases p with ⟨tp, P⟩
  rcases q with ⟨tq, Q⟩
  ...
  have hfInjective : Function.Injective
      (fun e : {e : σ.LengthTwoIndex //
          OpConjecture.GabrielArrowBridge.LengthTwo.target σ e = tp} ↦
        (OpConjecture.GabrielArrowBridge.LengthTwo.source σ e.1).1) := by
    intro e e' hee'
    apply Subtype.ext
    apply
  ...
  have hPQ : P = Q := by
    ...
    · intro heP
      have himage :
          (OpConjecture.GabrielArrowBridge.LengthTwo.source σ e.1).1 ∈
            σ.commonTargetPairSourceSet
              (OpConjecture.GabrielArrowBridge.LengthTwo.source σ)
              (OpConjecture.GabrielArrowBridge.LengthTwo.target σ)
              ⟨tp, P⟩ := by
...
theorem natCard_commonTargetPair_le_levelCount_three
  ...
  rw [OpConjecture.BottomLevels.BottomThreeAdapter.levelCount_three_eq_natCard_closedLevelThree]
...
def constructedActualClosedCommonTargetForkRealization
    ...
    ∀ {ι : Type u} [Finite ι]
      ...
      ActualClosedCommonTargetForkRealization σ
        (OpConjecture.GabrielArrowBridge.LengthTwo.source σ)
        (OpConjecture.GabrielArrowBridge.LengthTwo.target σ) := by
  letI : IsNoetherianRing Bᵐᵒᵖ :=
    OpConjecture.isNoetherianRing_op_of_finiteDimensional K B
  letI : IsArtinianRing Bᵐᵒᵖ :=
    OpConjecture.isArtinianRing_op_of_finiteDimensional K B
  ...
  exact {
    ...
    closed := fun p ↦
      OpConjecture.YonedaTwoSourceReduction.qClosure_isClosed_twoTopQuotientShapeSupport_actual
...
end OpConjecture.FamilyFiveCommonTargetConstruction
