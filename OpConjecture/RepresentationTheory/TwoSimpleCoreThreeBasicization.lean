...
namespace OpConjecture.TwoSimpleCoreThreeBasicization
...
theorem chosenBasicModel_isBlockConnected
  ...
  exact
    OpConjecture.FullIdempotentMorita.fullCorner_isBlockConnected
      (chosenBasicizingFullIdempotent (K := K) A).idem
      (OpConjecture.FullIdempotentMorita.FullFrame.ofFull
...
theorem support_trichotomy
  ...
  have hCoreRho :
      (quotientCore rho : Set
        (OpConjecture.CanonicalIndecomposableIndex.{u, u} P.Carrier)).ncard =
          3 :=
  ...
  exact
    OpConjecture.CoordinateTwoVertexTrichotomy.trichotomy_of_basic_of_blockConnected
      tau P.basic hRingelTau hRank hCoreTau hNoParallel hBasicConnected

end OpConjecture.TwoSimpleCoreThreeBasicization
