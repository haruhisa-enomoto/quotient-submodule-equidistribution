...
namespace OpConjecture.CoordinateGabrielRealization
...
abbrev CoordinateData :=
  OpConjecture.CotangentExtBridge.CoordinateData
    (K := K) (B := B) (I := I)
...
noncomputable def cyclicIdentityIso (c : B) :
    OpConjecture.Tsukamoto.cyclicRightIdealFG c ≅
      FGModuleCat.of Bᵐᵒᵖ
        (OpConjecture.Tsukamoto.principalRightIdeal c) :=
  Iso.refl _
  ...
  · intro x hx
    ...
    have hsmul :
        op (unop x.1) • xₑ ∈
          Ring.jacobson Bᵐᵒᵖ •
            (⊤ : Submodule Bᵐᵒᵖ
              (OpConjecture.Tsukamoto.principalRightIdeal e)) :=
...
noncomputable def vertexTopCyclicLinearEquiv (i : I) :
  ...
  letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule i
  exact
    (OpConjecture.IndecomposableSkeleton.FaithfulCore.ConnectedSmallCore.SplitBasicGabrielArrowRealization.moduleTopLinearEquivOfIso
...
def cornerKernelElement
  ...
  let p : OpConjecture.Tsukamoto.principalRightIdeal
      (D.liftedCoordinate i) :=
    ⟨op c,
...
def cyclicToVertexKernelLinearMap
    ...
    (hcJ : c ∈ Ring.jacobson B)
    (hleft : D.liftedCoordinate i * c = c) :
    OpConjecture.Tsukamoto.principalRightIdeal c →ₗ[Bᵐᵒᵖ]
  ...
  toFun x :=
    ⟨⟨x.1, D.principalRightIdeal_le_coordinate hleft x.property⟩, by
      ...
      exact D.coordinateCharacter_eq_zero_of_mem_jacobson i
        (OpConjecture.Tsukamoto.unop_mem_jacobson_of_mem_principalRightIdeal
          hcJ x.property)⟩
  map_add' _ _ := rfl
  map_smul' _ _ := rfl
...
noncomputable def detectingFunctional
  ...
  (OpConjecture.CotangentExtBridge.CoordinateData.exists_cotangent_functional
    (K := K) hc2).choose
...
def cornerTopLinearMap
    ...
    OpConjecture.Tsukamoto.principalRightIdeal c →ₗ[Bᵐᵒᵖ] K := by
  letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule j
  exact
...
theorem cornerTopLinearMap_surjective
  ...
  letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule j
  let g :
      OpConjecture.Tsukamoto.principalRightModule c ⟶
  ...
  have hg : g ≠ 0 := by
    ...
    have hhom := congrArg
      (fun q : OpConjecture.Tsukamoto.principalRightModule c ⟶
...
theorem ker_cornerTopLinearMap_eq_jacobson
    ...
    LinearMap.ker (D.cornerTopLinearMap i j hcJ hleft hc2) =
      Module.jacobson Bᵐᵒᵖ
        (OpConjecture.Tsukamoto.principalRightIdeal c) := by
  letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule j
  letI : IsSimpleModule Bᵐᵒᵖ K :=
    D.coordinateSimple_isSimpleModule j
  let g := D.cornerTopLinearMap i j hcJ hleft hc2
  let xc : OpConjecture.Tsukamoto.principalRightIdeal c :=
    ⟨op c, Ideal.mem_span_singleton_self (op c)⟩
  have hgenc : g xc ≠ 0 :=
  ...
  · exact
      IsSemisimpleModule.jacobson_le_ker Bᵐᵒᵖ Bᵐᵒᵖ
        (OpConjecture.Tsukamoto.principalRightIdeal c) K g
...
noncomputable def cornerTopCyclicLinearEquiv
    ...
    (hc2 : c ∉ (Ring.jacobson B) ^ 2) :
    letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule j
    ((OpConjecture.Tsukamoto.cyclicRightIdealFG c : Type u) ⧸
      Module.jacobson Bᵐᵒᵖ
        (OpConjecture.Tsukamoto.cyclicRightIdealFG c)) ≃ₗ[Bᵐᵒᵖ] K := by
  letI : Module Bᵐᵒᵖ K := D.coordinateSimpleModule j
  exact
    (OpConjecture.IndecomposableSkeleton.FaithfulCore.ConnectedSmallCore.SplitBasicGabrielArrowRealization.moduleTopLinearEquivOfIso
...
variable {K B : Type u} {I : Type v} {kappa : Type w}
  ...
  (tau : OpConjecture.IndecomposableSkeleton.{u, w, u} Bᵐᵒᵖ kappa)
...
end OpConjecture.CoordinateGabrielRealization
