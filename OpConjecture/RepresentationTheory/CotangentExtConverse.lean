...
def vertexKernelElementOfJacobian
  ...
  let p : OpConjecture.Tsukamoto.principalRightIdeal e :=
    ⟨op (e * z), by
...
theorem exists_aligned_cross_cotangent_of_nontrivial_ext_one
    ...
    (hExt : Nontrivial
      (OpConjecture.GabrielArrowBridge.ExtOne sigma s t)) :
...
theorem exists_aligned_cross_cotangent_of_gabrielArrow
    (a : OpConjecture.GabrielArrowBridge.GabrielArrowIndex sigma) :
...
theorem exists_aligned_cross_cotangent_of_extGabrielArrow
    (a : OpConjecture.GabrielArrowBridge.ExtGabrielArrowIndex
      (K := K) sigma) :
