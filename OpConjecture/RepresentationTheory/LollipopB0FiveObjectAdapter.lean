...
namespace OpConjecture.LollipopConcrete.ModuleLayer.FiveObjectAdapter
...
abbrev Label :=
  OpConjecture.BottomLevels.LollipopRelationTables.DeadPathLabel
...
structure Classification where
  exhaustive :
    ∀ M : FGModuleCat (B0Model K),
      OpConjecture.Foundation.IsIndecomposableModule (B0Model K) M →
...
theorem obj_indecomposable (i : Label) :
    OpConjecture.Foundation.IsIndecomposableModule (B0Model K) (obj K i) := by
  cases i with
...
end OpConjecture.LollipopConcrete.ModuleLayer.FiveObjectAdapter
