...
theorem indecomposable_map {M : FGModuleCat.{wR} R}
    ...
    OpConjecture.Foundation.IsIndecomposableModule S (E.functor.obj M) := by
...
theorem indecomposable_map_iff {M : FGModuleCat.{wR} R} :
    OpConjecture.Foundation.IsIndecomposableModule S (E.functor.obj M) ↔
...
def alignedFgEquivalence
    ...
    (σ : OpConjecture.IndecomposableSkeleton
      A ι)
    (τ : OpConjecture.IndecomposableSkeleton
      B κ) :
