...
theorem radicalRestriction_surjective
  ...
  have hy :
    ...
    rw [OpConjecture.LengthTwoGabrielBridge.map_jacobson_of_surjective_of_isArtinian
      f hf]
    exact y.2
  obtain ⟨z, hz, hzy⟩ := hy
  refine ⟨⟨z, hz⟩, ?_⟩
...
theorem moduleTop_sumOver_isIsotypicOfType
  ...
  have hproduct :
    ...
    OpConjecture.LengthTwoGabrielBridge.isIsotypicOfType_pi hcomponent
  ...
  have hproduct :
      IsIsotypicOfType R
        (∀ _ : J,
          Module.jacobson R (Module.jacobson R (σ.obj x)))
        (Module.jacobson R (Module.jacobson R (σ.obj x))) :=
    OpConjecture.LengthTwoGabrielBridge.isIsotypicOfType_pi
...
theorem middleLoewyLayer_isIsotypicOfType_of_inAdd_lengthTwoPair
  ...
  have hsum :
    ...
    exact
      OpConjecture.LengthTwoGabrielBridge.IndecomposableSkeleton.moduleTop_isIsotypicOfType_of_inFac_length_two_pair
...
theorem secondModuleRadical_isIsotypic_of_inFac_quotientChain
  ...
  exact
    OpConjecture.LengthTwoGabrielBridge.IsIsotypicOfType.of_surjective_of_semisimple
      hsource
      (radicalRestriction (radicalRestriction P.map.hom.hom))
      P.secondRadical_surjective
