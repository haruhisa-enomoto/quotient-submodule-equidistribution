...
namespace OpConjecture.ProductModules
...
theorem indecomposable_leftPart_or_rightPart_subsingleton
    (M : ModuleCat.{v} (A × B))
    (hM : OpConjecture.Foundation.IsIndecomposableModule (A × B) M) :
...
theorem left_indecomposable_of_product
    ...
    (h : letI := productModule X Y
      OpConjecture.Foundation.IsIndecomposableModule (A × B) (X × Y)) :
    OpConjecture.Foundation.IsIndecomposableModule A X := by
  letI := productModule X Y
...
theorem right_indecomposable_of_product
    ...
    (h : letI := productModule X Y
      OpConjecture.Foundation.IsIndecomposableModule (A × B) (X × Y)) :
    OpConjecture.Foundation.IsIndecomposableModule B Y := by
  letI := productModule X Y
  rw [OpConjecture.Foundation.isIndecomposableModule_iff_nontrivial_and_forall_isIdempotentElem]
  constructor
  · apply not_subsingleton_iff_nontrivial.mp
...
theorem product_indecomposable_of_left
    (X : ModuleCat.{v} A) (Y : ModuleCat.{v} B)
    (hX : OpConjecture.Foundation.IsIndecomposableModule A X)
    (hY : Subsingleton Y) :
    letI := productModule X Y
    OpConjecture.Foundation.IsIndecomposableModule (A × B) (X × Y) := by
  letI := productModule X Y
  rw [OpConjecture.Foundation.isIndecomposableModule_iff_nontrivial_and_forall_isIdempotentElem]
...
theorem product_indecomposable_of_right
    (X : ModuleCat.{v} A) (Y : ModuleCat.{v} B)
    (hX : Subsingleton X)
    (hY : OpConjecture.Foundation.IsIndecomposableModule B Y) :
    letI := productModule X Y
    OpConjecture.Foundation.IsIndecomposableModule (A × B) (X × Y) := by
  letI := productModule X Y
  rw [OpConjecture.Foundation.isIndecomposableModule_iff_nontrivial_and_forall_isIdempotentElem]
...
theorem indecomposable_component_classification
    (M : ModuleCat.{v} (A × B))
    (hM : OpConjecture.Foundation.IsIndecomposableModule (A × B) M) :
    (OpConjecture.Foundation.IsIndecomposableModule A (leftPart M) ∧
        Subsingleton (rightPart M)) ∨
      (Subsingleton (leftPart M) ∧
        OpConjecture.Foundation.IsIndecomposableModule B (rightPart M)) := by
  have hsplit :=
    @OpConjecture.Foundation.IsIndecomposableModule.of_linearEquiv
...
theorem leftEmbedding_indecomposable (X : FGModuleCat.{v} A)
    (hX : OpConjecture.Foundation.IsIndecomposableModule A X) :
    OpConjecture.Foundation.IsIndecomposableModule (A × B)
...
end OpConjecture.ProductModules
