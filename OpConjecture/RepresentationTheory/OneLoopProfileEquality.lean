...
theorem rightEquidistribution_of_algEquivComm
  ...
  exact
    OpConjecture.rightEquidistribution_of_moritaEquivalence_op K B
...
theorem rightSkeletonEquidistribution_of_algEquivComm
    ...
    ∀ {iota : Type v} [Finite iota]
      (sigma :
        OpConjecture.IndecomposableSkeleton.{u, v, u} Bᵐᵒᵖ iota),
  ...
  intro iota _ sigma
  let tau :=
    OpConjecture.rightIndecomposableSkeleton.{u, u, u} K B
  let E :
      OpConjecture.IndecomposableSkeleton.AlignedEquivalence sigma tau :=
    OpConjecture.IndecomposableSkeleton.Equivalence.alignedEquivalence
      sigma tau CategoryTheory.Equivalence.refl
  letI : Finite
      (OpConjecture.CanonicalIndecomposableIndex.{u, u} Bᵐᵒᵖ) :=
...
theorem skeletonEquidistribution_of_algEquivComm
    ...
    ∀ {iota : Type v} [Finite iota]
      (sigma :
        OpConjecture.IndecomposableSkeleton.{u, v, u} B iota),
      sigma.qClosure.levelPolynomial =
        sigma.sClosure.levelPolynomial := by
  letI : IsArtinianRing B := IsArtinianRing.of_finite K B
  letI : IsNoetherianRing B := inferInstance
  ...
  letI : Finite
    ...
    E.labelEquiv.finite_iff.mp inferInstance
  let D :=
    OpConjecture.Contragredient.alignedBiduality K B sigma tau
  have hdual :
      sigma.qClosure.levelPolynomial =
        tau.sClosure.levelPolynomial :=
...
theorem skeletonEquidistribution_of_surjective_oneLoop
    ...
    [Field K]
    [Ring B] [Algebra K B] [FiniteDimensional K B]
    (f : OpConjecture.Foundation.pathAlgebra K OpConjecture.Foundation.Quiver.OneLoop →ₐ[K] B)
...
theorem algebraNodeEquidistribution_of_oneLoopPresentation
    {K : Type u} [Field K]
    (B :
      OpConjecture.BottomLevels.FiniteDimensionalRecurrence.AlgebraNode K)
    (P : OneLoopPresentation K B.Carrier) :
    (OpConjecture.BottomLevels.FiniteDimensionalRecurrence.AlgebraNode.qClosure
        K B).levelPolynomial =
      (OpConjecture.BottomLevels.FiniteDimensionalRecurrence.AlgebraNode.sClosure
