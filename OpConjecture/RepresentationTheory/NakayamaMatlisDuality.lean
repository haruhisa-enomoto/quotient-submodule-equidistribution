...
theorem ker_ordinaryBoundaryLinear
  ...
  · intro c hc
    ...
    have hphi : phi ∈ OpConjecture.CoefficientDual.annihilator (E := E)
        (LinearMap.ker (P.differentialPrecompLinear (k := k) Y)) := by
      rw [← P.range_augmentationPrecompLinear (k := k) Y]
      rw [OpConjecture.CoefficientDual.mem_annihilator_iff]
      intro f hf
      obtain ⟨g, rfl⟩ := hf
    ...
    have hphiRange : phi ∈ LinearMap.range
      ...
      rw [OpConjecture.CoefficientDual.range_lcomp_eq_annihilator_ker]
      exact hphi
...
theorem boundaryStableLinear_surjective
  ...
  let psi₀ : (P.augmentation.p ⟶ Q.p) →ₗ[k] E :=
    N.equiv₀ Q.p (Q.f ≫ a₀)
  have hpsi₀ : psi₀ ∈ OpConjecture.CoefficientDual.annihilator (E := E)
