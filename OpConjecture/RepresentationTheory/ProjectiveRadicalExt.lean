...
namespace OpConjecture.ProjectiveRadicalExt
...
theorem exists_surjective_to_chosen_simple
  ...
  have hSIndec : OpConjecture.Foundation.IsIndecomposableModule A S :=
...
theorem exists_epi_to_biprod_chosen_simples_of_not_indec
    ...
    [AddCommGroup M] [Module A M] [Module.Finite A M]
    [Nontrivial M]
    (hNotIndec : ¬ OpConjecture.Foundation.IsIndecomposableModule A M) :
    ∃ p q : tau.SimpleIndex,
  ...
  have hnotAll :
    ...
    intro hall
    exact hNotIndec
      (OpConjecture.Foundation.isIndecomposableModule_of_forall_isCompl hall)
  push Not at hnotAll
...
theorem outgoing_of_decomposable_radical
    ...
    (hNotIndec :
      ¬ OpConjecture.Foundation.IsIndecomposableModule A (tau.moduleRadical i)) :
    HasOutgoingExtGabrielFork (K := K) tau := by
...
end OpConjecture.ProjectiveRadicalExt
