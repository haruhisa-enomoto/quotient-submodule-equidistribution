...
def injectiveIndexUnique (h₁ : OneSimple σ) :
  ...
  letI : Unique
      {i : ι // i ∈ OpConjecture.RingelStable.projectiveSet σ} := by
    change Unique
      (OpConjecture.IndecomposableSkeleton.ProjectiveSimpleRank.ProjectiveIndex
        σ)
    infer_instance
  let e :=
    OpConjecture.RingelStable.projectiveInjectiveLabelEquiv
      (R := R) K σ
  let p₀ : {i : ι // i ∈ OpConjecture.RingelStable.projectiveSet σ} :=
    default
  let i₀ := e p₀
  exact
    { default := i₀
...
theorem extGabrielArrowIndex_subsingleton
    ...
    Subsingleton
      (OpConjecture.GabrielArrowBridge.ExtGabrielArrowIndex (K := K) σ) := by
  letI : Unique σ.SimpleIndex := simpleIndexUnique σ h₁
  constructor
  rintro ⟨s, t, a⟩ ⟨s', t', b⟩
...
def OneVertexAtMostOneLoopNakayamaClassification : Prop :=
  OneSimple σ →
    Subsingleton
      (OpConjecture.GabrielArrowBridge.ExtGabrielArrowIndex (K := K) σ) →
