...
theorem indecomposable_inflation (I : TwoSidedIdeal R)
    (X : FGModuleCat.{u} (Quotient.Factor I))
    (hX : OpConjecture.Foundation.IsIndecomposableModule (Quotient.Factor I) X) :
    OpConjecture.Foundation.IsIndecomposableModule R ((Quotient.functor I).obj X) := by
...
theorem indecomposable_of_inflation (I : TwoSidedIdeal R)
    (X : FGModuleCat.{u} (Quotient.Factor I))
    (hX : OpConjecture.Foundation.IsIndecomposableModule R ((Quotient.functor I).obj X)) :
    OpConjecture.Foundation.IsIndecomposableModule (Quotient.Factor I) X := by
  rw [OpConjecture.Foundation.isIndecomposableModule_iff_nontrivial_and_forall_isIdempotentElem]
  refine ⟨hX.nontrivial, ?_⟩
  intro f hf
...
def descendFG (I : TwoSidedIdeal R) (X : FGModuleCat.{u} R)
  ...
  letI : Module (Quotient.Factor I) X :=
    Function.Surjective.moduleLeft
      (OpConjecture.AnnihilatorInflation.Quotient.map I)
...
theorem inflation_covers_killed_label {k : iota}
  ...
  have hXin : OpConjecture.Foundation.IsIndecomposableModule R
  ...
  have hX : OpConjecture.Foundation.IsIndecomposableModule (Quotient.Factor I) X :=
...
theorem ideal_le_supportAnnihilator_image
  ...
  intro r hr
  change OpConjecture.AnnihilatorInflation.Quotient.map I r ∈
