...
namespace OpConjecture.RadicalTopExtFork
...
theorem outgoing_of_nonsimple_radical_top
  ...
  have htopSurj : Function.Surjective topMap.hom.hom :=
    (OpConjecture.IndecomposableSkeleton.fg_epi_iff_surjective topMap).mp
      inferInstance
  let eTopP :
      (tau.obj proj ⧸ Module.jacobson A (tau.obj proj)) ≃ₗ[A]
        tau.obj s.1 :=
    OpConjecture.ProjectiveSimpleTop.topLinearEquivOfSurjectiveToSimple
  ...
  have hfSurj : Function.Surjective f.hom.hom :=
    (OpConjecture.IndecomposableSkeleton.fg_epi_iff_surjective f).mp hf
  let rMap : tau.moduleRadical proj →ₗ[A] tau.moduleRadical i :=
    OpConjecture.ProjectiveRadicalExt.radicalMapOfSurjective f.hom.hom
  ...
  have hJTopNotIndec : ¬ OpConjecture.Foundation.IsIndecomposableModule A JTop := by
    intro hIndec
    apply hRadicalTop
    exact
  ...
  obtain ⟨p, q, radTopQuot, hradTopQuot⟩ :=
    OpConjecture.ProjectiveRadicalExt.exists_epi_to_biprod_chosen_simples_of_not_indec
      tau hJTopNotIndec
  ...
  · subst q
    exact
      OpConjecture.ProjectiveRadicalExt.outgoing_parallel_extGabrielFork_of_projective_radical_quotient
        tau hFinite s p (P := tau.obj proj)
          (radQuot := radQuotP') eTopP
  · exact
      OpConjecture.ProjectiveRadicalExt.outgoing_extGabrielFork_of_projective_radical_quotient_distinct
...
end OpConjecture.RadicalTopExtFork
