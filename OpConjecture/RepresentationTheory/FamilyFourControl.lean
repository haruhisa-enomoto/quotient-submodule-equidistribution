...
namespace OpConjecture.FamilyFourControl
...
theorem isIndecomposableModule_of_simple_top
  ...
  apply OpConjecture.Foundation.isIndecomposableModule_of_forall_isCompl
  intro P Q hPQ
  by_cases hPtop : P = ⊤
  · right
    ...
    ∃ N : Submodule R M,
      N ≤ Module.jacobson R M ∧
        Module.length R (M ⧸ N) = 3 ∧
          OpConjecture.Foundation.IsIndecomposableModule R (M ⧸ N) := by
  let J : Submodule R M := Module.jacobson R M
...
theorem isUniserialModule_of_isIndecomposable_of_length_eq_two
    {M : Type v} [AddCommGroup M] [Module R M]
    [IsNoetherian R M] [IsArtinian R M]
    (hindec : OpConjecture.Foundation.IsIndecomposableModule R M)
    (hlength : Module.length R M = 2) :
    IsUniserialModule R M := by
  let J : Submodule R M := Module.jacobson R M
  have htop : IsSimpleModule R (M ⧸ J) :=
    OpConjecture.BottomTwoSimpleTop.simple_top_of_isIndecomposableModule_of_length_eq_two
      hindec hlength
  have hJne : J ≠ ⊥ :=
    OpConjecture.BottomTwoSimpleTop.jacobson_ne_bot_of_isIndecomposableModule_of_length_eq_two
      hindec hlength
...
theorem isUniserialModule_of_length_eq_three_of_simple_top_of_radical_indec
    ...
    (htop : IsSimpleModule R (M ⧸ Module.jacobson R M))
    (hradIndec :
      OpConjecture.Foundation.IsIndecomposableModule R (Module.jacobson R M)) :
    IsUniserialModule R M := by
  let J : Submodule R M := Module.jacobson R M
...
theorem simple_radical_submodule_quotient
    ...
    Module.length R (M ⧸ P) = 2 ∧
      OpConjecture.Foundation.IsIndecomposableModule R (M ⧸ P) := by
  let J : Submodule R M := Module.jacobson R M
...
noncomputable def simpleSubmodulesLinearEquiv_of_indec_length_two
    ...
    (hYindec : OpConjecture.Foundation.IsIndecomposableModule R Y)
...
theorem exists_repeatedRadicalFork_of_not_radical_indec
    ...
    (hradNotIndec :
      ¬ OpConjecture.Foundation.IsIndecomposableModule R (sigma.moduleRadical x)) :
...
def TwoNonsimpleLongTopRadicalControl : Prop :=
  ∀ {S : Set iota}, S.ncard = 3 → sigma.qClosure.IsClosed S →
    ∀ {x y s : iota},
      ...
      (sigma.compositionLength y ≠ 2 →
        IsSimpleModule R (sigma.moduleTop y) ∧
          OpConjecture.Foundation.IsIndecomposableModule R (sigma.moduleRadical y))
...
theorem impossible_two_copy_isotypic_radical
    (hnoParallel :
      OpConjecture.LoewyTwoRankCore.NoParallelExtOne sigma K)
    {j s t : iota}
...
theorem radical_isIndecomposable_of_simple_top_of_twoNonsimple
  ...
  letI : IsSimpleModule Aᵐᵒᵖ F.leftKernel :=
    F.leftKernel_simple
  have hPindec : OpConjecture.Foundation.IsIndecomposableModule Aᵐᵒᵖ Pfg :=
...
end OpConjecture.FamilyFourControl
