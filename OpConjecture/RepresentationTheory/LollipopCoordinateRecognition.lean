...
variable
  {D : OpConjecture.QuotientSurvival.TwoCoordinateData
    (K := K) (B := B)}
  {x a : B}
  (L : OpConjecture.QuotientSurvival.TwoCoordinateData.LollipopRadicalData
    D x a)
...
open OpConjecture.LollipopConcrete
...
def b1BaseAlgHom :
    OpConjecture.LollipopConcrete.B1.BaseAlgebra K →ₐ[K] B where
  toFun r :=
...
theorem b1BaseAlgHom_apply
    (r : OpConjecture.LollipopConcrete.B1.BaseAlgebra K) :
    L.b1BaseAlgHom r =
      r.1.fst • D.liftedCoordinate 0 + r.1.snd • x +
...
def b1ArrowLinearMap (a x : B) :
    OpConjecture.LollipopConcrete.B1.ArrowCorner K →ₗ[K] B where
...
theorem b1ArrowLinearMap_apply
    (m : OpConjecture.LollipopConcrete.B1.ArrowCorner K) :
    b1ArrowLinearMap a x m = m.fst • a + m.snd • (a * x) := rfl

include L in
/-- The coordinate algebra map from the live-path model. -/
def b1AlgHom : OpConjecture.LollipopConcrete.B1.B1Model K →ₐ[K] B := by
...
theorem b1AlgHom_apply
    (m : OpConjecture.LollipopConcrete.B1.B1Model K) :
    L.b1AlgHom m =
...
theorem b1AlgHom_bijective (hlive : a * x ≠ 0) :
  ...
  · intro b
    ...
    refine ⟨(((((c0, cx), c1), (ca, cu))) :
      OpConjecture.LollipopConcrete.B1.B1Model K), ?_⟩
    rw [L.b1AlgHom_apply]
    change
...
noncomputable def b1AlgEquiv (hlive : a * x ≠ 0) :
    OpConjecture.LollipopConcrete.B1.B1Model K ≃ₐ[K] B :=
