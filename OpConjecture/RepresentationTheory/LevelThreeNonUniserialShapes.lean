...
theorem qClosure_isClosed_gabrielEdgeExtraSimpleSupport
    ...
    ∀ {κ : Type x} [Finite κ]
      (τ : OpConjecture.IndecomposableSkeleton.{x, x, x} Aᵐᵒᵖ κ)
      (p : τ.EdgeExtraSimple
        (OpConjecture.GabrielArrowBridge.LengthTwo.source τ)),
      τ.qClosure.IsClosed
        (τ.edgeExtraSimpleSupport
          (OpConjecture.GabrielArrowBridge.LengthTwo.source τ) p) := by
  letI : IsNoetherianRing Aᵐᵒᵖ :=
    OpConjecture.isNoetherianRing_op_of_finiteDimensional K A
  intro κ _ τ p
  ...
  exact
    OpConjecture.LoewyTwoGabrielClassification.qClosure_isClosed_length_two_top_pair
      K A τ e.2
      (OpConjecture.GabrielArrowBridge.LengthTwo.quotient τ e)
      (OpConjecture.GabrielArrowBridge.LengthTwo.submodule τ e)
