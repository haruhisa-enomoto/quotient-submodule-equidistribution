...
def quotientBadRootedFourFamily : Finset (Finset ι) :=
  OpConjecture.SetClosure.badRootedDeletions
  ...
  invFun D := by
    have h :
      ...
      simpa only [quotientBadRootedFourFamily,
        OpConjecture.SetClosure.mem_badRootedDeletions] using D.2
...
noncomputable def quotientFourPacketDecompositionOfClassification
  ...
  let E := OpConjecture.FourVertexLadderPackets.decompositionOfFiberClassification
    Hook
...
theorem badRootedFour_card_eq_of_packetDecompositions
    ...
    (QD : OpConjecture.FourVertexLadderPackets.Decomposition
      (QuotientBadRootedFour (k := k) (R := R) σ)
      QHook QDouble QFixed QTriangle)
    (SD : OpConjecture.FourVertexLadderPackets.Decomposition
      (SubmoduleBadRootedFour (k := k) (S := S) σ τ D)
      SHook SDouble SFixed STriangle)
    (B : OpConjecture.FourVertexLadderPackets.ReversalBalance
      QHook QDouble QFixed QTriangle
      SHook SDouble SFixed STriangle) :
  ...
  have h :=
    OpConjecture.FourVertexLadderPackets.killed_card_eq_of_reversalBalance
      QD SD B
