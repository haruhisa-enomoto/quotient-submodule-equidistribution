...
theorem oppositeQuotientCoordinateHom_injective :
  ...
  rw [Ideal.Quotient.eq_zero_iff_mem]
  simpa using
    OpConjecture.CotangentExtBridge.CoordinateData.op_mem_jacobson_of_mem_jacobson
      (B := B) hb
...
namespace OpConjecture.OppositeCoordinateSupport
...
variable {K B : Type u}
  ...
  (sigma : OpConjecture.IndecomposableSkeleton.{u, v, u} Bᵐᵒᵖ kappa)
  (tau : OpConjecture.IndecomposableSkeleton.{u, v, u} (Bᵐᵒᵖ)ᵐᵒᵖ lambda)
  (A : OpConjecture.IndecomposableSkeleton.AlignedAntiEquivalence sigma tau)
  (D : OpConjecture.QuotientSurvival.TwoCoordinateData (K := K) (B := B))
...
def alignedOppositeCoordinateData :
    OpConjecture.QuotientSurvival.TwoCoordinateData
      (K := K) (B := Bᵐᵒᵖ) :=
...
def extArrowDualEquiv :
  ...
  (OpConjecture.GabrielArrowBridge.LengthTwo.lengthTwoEquivExtGabrielArrow
...
theorem source_extArrowDualEquiv
  ...
  rw [OpConjecture.ExtDualTransport.LengthTwo.source_lengthTwoEquivExtGabrielArrow]
  rw [OpConjecture.ExtDualTransport.AlignedAntiEquivalence.source_lengthTwoEquiv]
  apply congrArg
    (OpConjecture.ExtDualTransport.AlignedAntiEquivalence.simpleIndexEquiv
      sigma tau A)
  calc
    OpConjecture.GabrielArrowBridge.LengthTwo.target sigma (eSigma.symm a) =
...
theorem target_extArrowDualEquiv
  ...
  change ExtGabrielArrowIndex.target tau
      (eTau
        (OpConjecture.ExtDualTransport.AlignedAntiEquivalence.lengthTwoEquiv
          sigma tau A (eSigma.symm a))) = _
  rw [OpConjecture.ExtDualTransport.LengthTwo.target_lengthTwoEquivExtGabrielArrow]
  rw [OpConjecture.ExtDualTransport.AlignedAntiEquivalence.target_lengthTwoEquiv]
  apply congrArg
    (OpConjecture.ExtDualTransport.AlignedAntiEquivalence.simpleIndexEquiv
      sigma tau A)
  calc
    OpConjecture.GabrielArrowBridge.LengthTwo.source sigma (eSigma.symm a) =
...
theorem algEquiv_b0_or_b1_of_outgoingOneWayLollipop
    (BD : OpConjecture.IndecomposableSkeleton.AlignedBiduality sigma tau)
    (hTau : NoParallelExtSupport (K := K) tau)
    (hShape :
      (D.alignedExtTwoVertexSupport sigma hSigma).IsOneWayLollipopAt 0 1)
    (hOutgoing :
    ...
    (hRank :
      OpConjecture.IndecomposableSkeleton.FaithfulCore.ConnectedSmallCore.projectiveRank
        sigma = 2)
    (hCore :
      (OpConjecture.IndecomposableSkeleton.FaithfulCore.quotientCore sigma :
        Set kappa).ncard = 3) :
    Nonempty (OpConjecture.LollipopConcrete.B0Model K ≃ₐ[K] Bᵐᵒᵖ) ∨
      Nonempty (OpConjecture.LollipopConcrete.B1.B1Model K ≃ₐ[K] Bᵐᵒᵖ) := by
  ...
  have hCoreTau :
    ...
    calc
      (OpConjecture.IndecomposableSkeleton.FaithfulCore.quotientCore tau :
          Set lambda).ncard =
          (OpConjecture.IndecomposableSkeleton.FaithfulCore.submoduleCore sigma :
            Set kappa).ncard :=
  ...
  have hRingelTau :
    ...
    OpConjecture.RingelStable.FaithfulCoreAdapter.ringelCoreCardinality_of_ringelEtaStableEquivalence
      tau K
  exact
    OpConjecture.QuotientSurvival.TwoCoordinateData.algEquiv_b0_or_b1_of_incomingOneWayLollipop
...
theorem algEquiv_b0_or_b1_direct_or_opposite_of_oneWayLollipop
    ...
    ((Nonempty (OpConjecture.LollipopConcrete.B0Model K ≃ₐ[K] B) ∨
      ...
      (Nonempty (OpConjecture.LollipopConcrete.B0Model K ≃ₐ[K] Bᵐᵒᵖ) ∨
        Nonempty (OpConjecture.LollipopConcrete.B1.B1Model K ≃ₐ[K] Bᵐᵒᵖ))) := by
  rcases
      OpConjecture.QuotientSurvival.TwoCoordinateData.algEquiv_b0_or_b1_or_normalizedOutgoing_of_oneWayLollipop
        (K := K) (B := B) (kappa := kappa)
...
end OpConjecture.OppositeCoordinateSupport
