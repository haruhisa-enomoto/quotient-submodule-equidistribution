...
namespace OpConjecture.LevelTwoUnconditional
...
noncomputable def relabelIndecomposableSkeleton
    ...
    {κ : Type v} {ι₂ : Type w}
    (τ : OpConjecture.IndecomposableSkeleton.{u, v, u} R₂ κ)
    (e : ι₂ ≃ κ) :
    OpConjecture.IndecomposableSkeleton.{u, w, u} R₂ ι₂ where
  obj i := τ.obj (e i)
...
noncomputable def relabelAlignedEquivalence
    ...
    (τ : OpConjecture.IndecomposableSkeleton.{u, v, u} R₂ κ)
    (e : ι₂ ≃ κ) :
    OpConjecture.IndecomposableSkeleton.AlignedEquivalence
...
theorem moduleTop_isIsotypicOfType_of_hasUniqueSimpleQuotientType
  ...
  have hMindec : OpConjecture.Foundation.IsIndecomposableModule R M :=
...
theorem exists_indecomposable_quotient_with_simple_radical
  ...
  have hJne : J ≠ ⊥ := by
    ...
    have hxSimpleModule : IsSimpleModule R (σ.obj x) :=
      _root_.OpConjecture.IndecomposableSkeleton.isSimpleModule_of_semisimple_of_indecomposable
        (σ.indecomposable x)
    exact hx
      ((_root_.OpConjecture.IndecomposableSkeleton.simple_iff_isSimpleModule_fg
  ...
  let ePi :
    ...
    e ≪≫
      _root_.OpConjecture.IndecomposableSkeleton.biproductIsoPiFG
  ...
  have hgRadical :
    ...
    rw [← hradQuotient]
    exact
      _root_.OpConjecture.LengthTwoGabrielBridge.map_jacobson_of_surjective_of_isArtinian
        g.hom.hom hgSurj
...
theorem compositionLength_eq_two_of_qClosed_pair_of_isotypicLoewyTwo
  ...
  have hjLength : σ.compositionLength j = 2 := by
    ...
    calc
      ...
      _ = (2 : ℕ∞) := rfl
  exact
    OpConjecture.BottomTwoSimpleTop.IndecomposableSkeleton.compositionLength_eq_two_of_qClosed_pair_of_epi
...
noncomputable def qLevelTwoClassification_of_finiteDimensional_of_finiteSkeleton
  ...
  · intro z
    exact
      OpConjecture.LoewyTwoGabrielClassification.qClosure_isClosed_length_two_top_pair
        K A σ z.2 (top z)
...
noncomputable def sLevelTwoClassification_of_finiteDimensional_of_finiteSkeleton
  ...
  intro ι _ σ
  let τCanonical :=
    OpConjecture.rightIndecomposableSkeleton.{x, x, x} K Aᵐᵒᵖ
  let DCanonical :=
    OpConjecture.Contragredient.alignedBiduality
      K Aᵐᵒᵖ σ τCanonical
  let τ :
      OpConjecture.IndecomposableSkeleton.{x, x, x}
        (Aᵐᵒᵖ)ᵐᵒᵖ ι :=
    relabelIndecomposableSkeleton τCanonical
      DCanonical.forward.labelEquiv
  let D :=
    OpConjecture.Contragredient.alignedBiduality K Aᵐᵒᵖ σ τ
  let hclassification :
      OpConjecture.LengthTwoGabrielBridge.IndecomposableSkeleton.IsotypicLoewyTwoIndecomposablesHaveSimpleTop
        τ :=
    OpConjecture.LoewyTwoGabrielClassification.isotypicLoewyTwoIndecomposablesHaveSimpleTop
  ...
  · intro z s hclosed hz hs
    have hzDual :
      ...
      rw [←
        OpConjecture.LengthTwoGabrielBridge.IndecomposableSkeleton.AlignedAntiEquivalence.compositionLength_eq
          σ τ D.forward]
    ...
    have hclosedImage :=
      (OpConjecture.LengthTwoGabrielBridge.AlignedBiduality.sClosure_isClosed_iff_qClosure_image
  ...
  · intro z
    exact
      OpConjecture.LengthTwoGabrielBridge.IndecomposableSkeleton.AlignedBiduality.sClosure_isClosed_length_two_pair_of_dual_isotypicLoewyTwo
...
theorem qLevelCount_two_eq_sLevelCount_two_of_rightRepresentationFinite
    ...
    [Field K] [IsAlgClosed K]
    [Ring A] [Algebra K A] [FiniteDimensional K A]
    (hrep : OpConjecture.IsRightRepresentationFinite.{x, x, x} K A) :
    letI : IsNoetherianRing Aᵐᵒᵖ :=
      OpConjecture.isNoetherianRing_op_of_finiteDimensional K A
    letI : Finite
        (OpConjecture.CanonicalIndecomposableIndex.{x, x} Aᵐᵒᵖ) :=
      hrep
    let σ :=
      OpConjecture.rightIndecomposableSkeleton.{x, x, x} K A
    σ.qClosure.levelCount 2 = σ.sClosure.levelCount 2 := by
  letI : IsNoetherianRing Aᵐᵒᵖ :=
    OpConjecture.isNoetherianRing_op_of_finiteDimensional K A
  let κ :=
...
end OpConjecture.LevelTwoUnconditional
