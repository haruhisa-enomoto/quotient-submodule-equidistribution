...
theorem topSplitInjectiveIndices_card_eq_natCard_simpleIndex :
  ...
  calc
    ...
    _ = (OpConjecture.RingelStable.projectiveSet σ).ncard :=
      (OpConjecture.RingelStable.projectiveSet_ncard_eq_injectiveSet_ncard
        (R := R) K σ).symm
...
theorem qLevelCount_card_sub_one_eq_simpleIndex
  ...
  calc
    σ.qClosure.levelCount (Nat.card ι - 1) =
      ...
      OpConjecture.SetClosure.levelCount_card_sub_one_eq_natCard_topExtreme
        σ.qClosure hcard
    _ = σ.qClosure.topExtremeFinset.card :=
      OpConjecture.SetClosure.natCard_topExtreme_eq_topExtremeFinset_card
        σ.qClosure
