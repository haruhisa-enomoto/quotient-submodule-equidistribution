import OpConjecture.Foundation.RepresentationTheory.Quiver.Representation.Simple
...
namespace OpConjecture.LollipopConcrete.QuiverLayer
...
def rep (V1 V2 : ModuleCat K) (loop : V1 ⟶ V1) (stem : V1 ⟶ V2) :
    OpConjecture.Foundation.QuiverRep K Vertex :=
  Paths.lift
...
structure IsB0Representation (M : OpConjecture.Foundation.QuiverRep K Vertex) : Prop where
...
abbrev S1 : OpConjecture.Foundation.QuiverRep K Vertex := OpConjecture.Foundation.simpleRep K Vertex .one
abbrev S2 : OpConjecture.Foundation.QuiverRep K Vertex := OpConjecture.Foundation.simpleRep K Vertex .two

/-- The two-dimensional nilpotent-loop representation. -/
def X : OpConjecture.Foundation.QuiverRep K Vertex :=
  rep K (K2 K) (0 : ModuleCat K) (jordan K) 0

/-- The one-dimensional arrow representation. -/
def A : OpConjecture.Foundation.QuiverRep K Vertex :=
  rep K (K1 K) (K1 K) 0 (𝟙 _)

/-- The fork-shaped representation at vertex `1`. -/
def P : OpConjecture.Foundation.QuiverRep K Vertex :=
...
theorem hom_s2_eq_zero_of_epi_stem
    (M : OpConjecture.Foundation.QuiverRep K Vertex)
    [Epi (M.map stemPath)]
    (f : M ⟶ S2 K) : f = 0 := by
  rw [OpConjecture.Foundation.hom_simpleRep_eq_zero_iff]
...
theorem hom_S1_S2_eq_zero (f : S1 K ⟶ S2 K) : f = 0 := by
  letI : Epi ((S1 K).map stemPath) :=
    (OpConjecture.Foundation.isZero_simpleRep_obj (k := K) (i := Vertex.one)
...
def quotientPRowRep : QuotientPRowLabel → OpConjecture.Foundation.QuiverRep K Vertex
  | .s1 => S1 K
  | .x => X K
  | .a => A K
  | .p => P K
...
end OpConjecture.LollipopConcrete.QuiverLayer
