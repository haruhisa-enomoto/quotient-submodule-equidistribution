...
theorem separatedCandidateFG_indecomposable
    ...
    (i : Nonsimple ⊕ (Vertex ⊕ Vertex)) :
    OpConjecture.Foundation.IsIndecomposableModule (Algebra S J)
      (separatedCandidateFG (sigma := sigma) i) := by
  rcases i with n | (a | a)
  · let X := sigma.obj (Sum.inl n)
    let D := moduleSeparatedData (S := S) (J := J) X.obj
    letI : Module (Algebra S J) (Realized D) := realizedModule D
    change OpConjecture.Foundation.IsIndecomposableModule (Algebra S J) (Realized D)
...
theorem exists_separatedCandidateFG_iso
  ...
  · letI : Subsingleton D.top := htop
    have hRadIndec : OpConjecture.Foundation.IsIndecomposableModule S D.radical :=
    ...
    have hRecSimpleModule :
      ...
      (simple_iff_isSimpleModule' (reconstructedModuleCat D)).1
        hRecSimpleCat
    have hRecIndec : OpConjecture.Foundation.IsIndecomposableModule (TrivSqZeroExt S J)
  ...
  · letI : Nontrivial D.top := htop
    ...
    · letI : Subsingleton D.radical := hrad
      have hTopIndec : OpConjecture.Foundation.IsIndecomposableModule S D.top :=
    ...
    · letI : Nontrivial D.radical := hrad
      ...
      have hRecIndecFG : OpConjecture.Foundation.IsIndecomposableModule
          (TrivSqZeroExt S J)
