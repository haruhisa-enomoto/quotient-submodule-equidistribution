...
theorem finrank_euler
    ...
    Module.finrank K (obj a ⟶ left x) +
      ...
      Module.finrank K (obj a ⟶ middle x) +
        (if a = x then 1 else 0) := by
  let f := OpConjecture.RepresentationDirected.postcompLinearMap
    (K := K) (X := obj a) (E.nu x)
  let g := OpConjecture.RepresentationDirected.postcompLinearMap
    (K := K) (X := obj a) (E.mu x)
...
open OpConjecture.RepresentationDirected.MeshExactness
