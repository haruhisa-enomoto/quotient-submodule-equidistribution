...
namespace OpConjecture.RepresentationDirected.DualDirectedOrder
...
theorem undualMap_ne_zero
    ...
    (hg : g ≠ 0) :
    OpConjecture.DualFixedSocleTransport.undualMap sigma tau D g ≠ 0 := by
  intro hz
  apply hg
  let h := (D.objIso j).hom ≫ g ≫ (D.objIso i).inv
  have hpre : D.categoryEquiv.functor.preimage h = 0 := by
    apply Quiver.Hom.unop_inj
    simpa [h, OpConjecture.DualFixedSocleTransport.undualMap] using hz
...
theorem isIso_undualMap_iff
    {i j : I}
    (g : tau.obj (D.labelEquiv j) ⟶ tau.obj (D.labelEquiv i)) :
    IsIso (OpConjecture.DualFixedSocleTransport.undualMap sigma tau D g) ↔
      IsIso g := by
  let h := (D.objIso j).hom ≫ g ≫ (D.objIso i).inv
...
theorem dual_homOrderProperty [LinearOrder I]
  ...
  let g : sigma.obj b ⟶ sigma.obj a :=
    OpConjecture.DualFixedSocleTransport.undualMap sigma tau D f
...
end OpConjecture.RepresentationDirected.DualDirectedOrder
